...
| `docs/...`、`README.md`、`CLAUDE.md` 等 | 改/删 | 删迁移专属 spec，更新引用 |
...
Run: `npx vitest run packages/mcp/tests/index.test.ts`
...
Run: `npx vitest run packages/core/tests/store-startup-validation.test.ts`
...
```bash
git add packages/core/tests packages/server/tests/routes.test.ts packages/mcp/tests/tools.test.ts
...
```bash
git add -A docs README.md CLAUDE.md packages/desktop/VERIFICATION.md
...
```bash
git grep -n "validateStrictStoreReadModels\|recoverPendingProductDeletes\|normalizeKind\|normalizeFormaExtension"
```
Expected: 均有命中（保留清单未被误删）。
...
- 文档（删 01/03、更新 index/02/04/05/08/10/11/12 + 活文档 + desktop/VERIFICATION）→ Task 7 ✅
