...
- [ ] **Step 1: 从需求文档分支切出实施分支**

```bash
cd /Users/xubo/x-studio/forma
git checkout docs/hardening-requirements
git checkout -b feat/hardening-batch
git log --oneline -1   # 应为 b1a1b6f docs: apply review revisions...
```
...
git commit -m "fix(core): drop stdout console.log that corrupts MCP stdio framing (R10)"
```
...
git commit -m "fix(core): classify malformed data URL payloads as ARTIFACT_INVALID_INPUT (R4)"
```
...
> 总字节预算（MAX_TOTAL_ASSET_BYTES）不在测试中真实构造 48MiB 输入——用例会太慢。其检查逻辑与 count 检查同一代码路径（见 Step 3），由 count 用例覆盖结构，常量值由 typecheck 守护。
...
在 `localizeArtifactAssets` 的 return 之前（函数最后构造 LocalizeResult 处；找到 `return {` 返回 html/files/assets 的位置）加：

```typescript
  if (ctx.files.size > MAX_ASSET_COUNT) {
    throw new FormaError("ARTIFACT_INVALID_INPUT", `Artifact asset count exceeds the ${MAX_ASSET_COUNT} budget`, {
      ...
      actual: ctx.files.size,
    });
  }
  let totalAssetBytes = 0;
  for (const buf of ctx.files.values()) {
...
```bash
npx vitest run packages/core/tests/artifact-asset-pipeline.test.ts packages/core/tests/design-save.test.ts
```
...
```bash
git add packages/core/src/artifact-asset-pipeline.ts packages/core/tests/artifact-asset-pipeline.test.ts
git commit -m "feat(core): enforce HTML/asset budgets and sharp pixel limit at save entry (R3)"
```
...
git commit -m "fix(core): retry product id collisions and clean up failed creates (R2)"
```
...
git commit -m "refactor(server): route mutation-origin audit log through request.log (R6)"
```
...
git commit -m "feat(server): add read-only /api/health and switch desktop liveness probe to it (R5)"
```
...
**Files:**
- Modify: `packages/cli/src/index.ts`（4 处：spawn argv、parseForegroundServeArgs、defaultVerifyServerProcess、writeText mode）
- Modify: `packages/cli/tests/cli.test.ts`（foreground 用例、formaServerCommandLine helper、新增 0600 用例）
...
（options 形参保留 `token` 字段不动，调用方无需改。）

3. 新增用例（放在 serve describe 块内）：

```typescript
  it("serve start writes state files with 0600 permissions and keeps the token out of spawn argv", async () => {
    const { stat, writeFile: fsWriteFile, mkdir: fsMkdir } = await import("node:fs/promises");
...
```
Expected: 新 0600 用例 FAIL（默认 writeFile 无 mode）；foreground 用例 FAIL（`--serve-token` 已从参数移除但 parse 仍要求/或 parse 报 unexpected——以实际输出为准，进入 Step 3 修复）。
...
4. `resolveCliEnv` 中 writeText 默认实现（:522-527）：
...
git commit -m "fix(cli): keep serve token out of argv/ps, 0600 state files (R1)"
```
...
**Files:**
- Modify: `packages/core/src/artifact-static-validation.ts`（4 处加固）
- Modify: `packages/core/tests/artifact-static-validation.test.ts`（追加安全回归组）

**已知校验器缺口（核对源码确认）：** ① 无 meta refresh 规则；② `scanSvg` 不查 iframe/object/embed/foreignObject；③ `scanCssText` 只匹配 `url(...)`，漏 `image-set("...")` 裸字符串；④ `isJavascriptUrl` 不抗实体/控制字符混淆。`srcdoc`（iframe 整体被拒）与 `@font-face src url(...)`（url() 已覆盖）预期现状即通过。
...
```typescript
  ...
  // url() pattern above never sees. Flag remote and residual data: targets.
  const imageSetPattern = /(?:-webkit-)?image-set\s*\(\s*(['"])((?:https?:|data:|\/\/)[^'"]+)\1/gi;
...
git commit -m "feat(core): harden static validator against refresh/SVG-embed/image-set/obfuscated-js vectors (R7)"
```
...
git commit -m "feat(core): default preview renderer to sandboxed Chromium with explicit fallback gates (R11)"
```

---

### Task 10: F3(server) — artifact 详情响应增加 versions / current_version

**Files:**
- Modify: `packages/server/src/routes.ts:620-631`（artifact 详情路由）
...
npx vitest run packages/server/tests/routes.test.ts
```
Expected: 新用例 FAIL（响应无 versions 字段）。
...
git commit -m "feat(server): expose versions and current_version on artifact detail (F3)"
```
...
**Files:**
- Modify: `packages/web/src/api.ts`（ArtifactDetail 字段 + getArtifactVersionPreviewUrl）
...
- Modify: `packages/web/src/i18n.ts`（en + zh 各 5 个 key）
...
```typescript
    getArtifactVersionPreviewUrl: (productId, artifactId, version, resolution) =>
      `/api/products/${encodeURIComponent(productId)}/artifacts/${encodeURIComponent(artifactId)}/versions/${version}/preview/${resolution}`,
```
...
function previewUrl(productId: string, artifactId: string, version: number, resolution: "1x" | "2x"): string {
  return `/api/products/${productId}/artifacts/${artifactId}/versions/${version}/preview/${resolution}`;
}
...
```typescript
        // F3: artifacts with 2+ versions get a compare entry.
        const compareTargets = requirementArtifacts
          .filter((artifact) => (artifact.current_version ?? 1) >= 2)
...
`packages/web/src/pages/DesignView.test.tsx` 追加（用既有 fakeClient 模式，artifacts 带 `current_version: 2`）：

```typescript
  it("shows a compare entry for artifacts with 2+ versions (F3)", async () => {
    // 基于该文件既有的 render/fakeClient 辅助：listProductArtifacts 返回的 artifact
    // 增加 current_version: 2，断言文档中出现 compare 链接。
    // 具体复用方式以文件内既有 it() 的结构为准。
  });
```

> 注意：上面这个测试体要按 DesignView.test.tsx 既有 render 辅助真实写出——该文件的 fakeClient/render 模式在文件前 60 行已确认存在。断言目标：`container.querySelector('a[href*="/compare"]')` 非空且 href 包含 artifact id。
...
git commit -m "feat(web): side-by-side artifact version compare view (F3)"
```
...
export async function diagnoseWorkspace(options: { home: string }): Promise<WorkspaceDiagnosis> {
  ...
  try {
    ...
    findings.push(toFinding("index", error, {}));
    return { findings, products_checked: 0 };
  }

  for (const entry of products) {
    let requirements: Array<{ id: string }> = [];
    try {
      await store.products.getProduct(entry.id);
      requirements = await store.requirements.getRequirementHistory(entry.id);
    } catch (error) {
      findings.push(toFinding("schema", error, { product_id: entry.id }));
      continue;
    }
    for (const requirement of requirements) {
      try {
        await store.copy.getTranslations(entry.id, requirement.id);
      } catch (error) {
        findings.push(toFinding("schema", error, { product_id: entry.id, requirement_id: requirement.id }));
      }
    }
  }
...
function toFinding(
  kind: "schema" | "index",
  error: unknown,
  scope: { product_id?: string; requirement_id?: string },
): WorkspaceFinding {
  if (error instanceof FormaError) {
    return { kind, ...scope, error_code: error.code, message: error.message };
  }
  return {
    kind,
    ...scope,
...
git commit -m "feat(core): add read-only diagnoseWorkspace for forma doctor (F4)"
```
...
async function runDoctor(args: string[], env: RuntimeCliEnv, output: CliOutput): Promise<CliResult> {
  ...
  for (const finding of diagnosis.findings) {
    const scope = [finding.product_id, finding.requirement_id].filter(Boolean).join("/");
...
```bash
git add packages/cli/src/index.ts packages/cli/tests/cli.test.ts
git commit -m "feat(cli): add read-only forma doctor command (F4)"
```
...
- **已知的两处现场适配点**（非 placeholder，是依赖既有测试harness的对齐动作，executor 须按文件实况完成）：T11 Step 3 的 `LocaleProvider` 包装方式以 `LocaleContext.tsx` 实际导出为准；T11 Step 5 的 DesignView 测试体要复用该文件既有 render/fakeClient 辅助写完整断言。
