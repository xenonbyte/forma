...
| 02 | `2026-05-21-forma-v6-02-async-startup-design.md` | Async store/server/MCP startup with strict validation failure as a startup error | 00 | 60 |
| 04 | `2026-05-21-forma-v6-04-legacy-surface-removal-design.md` | Removal of old public tools, routes, templates, and docs surface | 02 | 6, 13, 21, 38, 56 |
...
| 10 | `2026-05-21-forma-v6-10-server-web-routes-design.md` | Requirement-level Server/Web APIs and Web route model | 08 | 5, 6, 7, 38, 54, 60 |
| 11 | `2026-05-21-forma-v6-11-ui-canvas-design.md` | LeaferJS scene canvas, UI state, accessibility, i18n | 10 | 5, 41-48, 52 |
| 12 | `2026-05-21-forma-v6-12-verification-design.md` | Final verification matrix and release gate checks | 02, 04-11 | all remaining acceptance items |

The dedicated legacy compatibility preparation and rewrite specs were retired by the 2026-06-05 removal spec. Current v6 implementation starts directly from strict read-model validation and does not define a separate data rewrite stage.
...
- `design_id` is not a v6 public or runtime design key. Under strict v6 schemas, it is invalid in runtime schemas, API payloads, MCP payloads, Web types, and agent prompts.
...
- Startup validates the current v6 read model directly. Legacy or incomplete data fails loudly instead of entering a reduced compatibility mode.
...
- Strict schema validation is the runtime entry gate; do not add compatibility rewrite tooling as a prerequisite.
...
- The active stage specs and this index exist under `docs/superpowers/specs/`.
