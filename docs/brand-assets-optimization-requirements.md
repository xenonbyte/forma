...
| # | 决策 | 取代/否决的方案 |
|---|---|---|
| D1 | 生图模型目录扩充：OpenAI 加 `gpt-image-2` + `gpt-image-1.5`（provider 内默认改 `gpt-image-1.5`）；Gemini 加 `gemini-3-pro-image` + `gemini-3.1-flash-image`（provider 内默认改 `gemini-3.1-flash-image`）；全局仍 volcengine 优先 | 默认改成最贵的旗舰（1.5/3.1-flash 性价比更适合默认，旗舰留给质量场景） |
...
| D7 | 应用图标**全量多版本**（产出物）：standard + Android(foreground/background/monochrome) + iOS(dark/tinted)，各变体齐全、各自尺寸集；**产出方式见 D16**（3 基图 + 本地派生，非每变体生图） | 仅尺寸规范化保持单母版（无法产出分层/单色/深色，不达需求） |
...
| D12 | `fm-*` 无描述重跑维持**全量重生成**语义，仅在重跑前增加一道"将重新生成/覆盖 X"的确认 | 改为勾选式增量（改 5 个模板交互，收益不足） |
...
| D15 | **孤儿记录清理**（已采纳）：新增最小 `delete_brand_asset(product_id, kind, name)` MCP 工具；fm-brand-assets 重跑时按计划清理"超出当前数量/已关闭样式"的旧 store-shot/banner/poster 记录（如 count 由 5 降 3，删 `*-4`/`*-5`） | 静默残留孤儿（manifest 与磁盘越积越多，画布显示已废弃资产）——当前**无任何删除路径**，save 仅按名覆盖 |
| D16 | **图标产出方式（用户优化，已采纳）**：AI 只生成 **3 张 1080² 基图**——`a` 透明背景 LOGO、`b` 纯色/渐变背景、`c` LOGO 缩于 666 安全区+透明背景；其余变体全部由 core 用 sharp **合成 / tint / 缩放派生**，零额外生图（映射见 §6.5）。一次 `save_brand_asset(app-icon)` 传基图 + 颜色，core 产出该平台整套变体记录并**整组替换**（图标天然无孤儿，不依赖 D15） | 每版本独立生图（mobile ~21 张生成+逐张视检，慢/贵/各版本不一致） |
...
`BrandAssetRecord` 新增两个字段（按真实语义建模，**非为兼容保留**）：

- `surface?: "android" | "ios"`——仅 mobile/tablet 存在；驱动画布分组。
- `variant?: string`——kind 内子类型：
  - app-icon：层（`standard` / `foreground` / `background` / `monochrome` / `dark` / `tinted`），**必填**。
  - poster：样式（`portrait` / `landscape` / `square`），**必填**。
  - store-shot / banner：无 `variant`（同 surface 下多张靠 `name` 区分）。

记录身份键仍为 `(kind, name)`（覆盖语义不变）；`name` 由 agent 确定性命名（`android-foreground` / `android-1` / `portrait`）。
...
|---|---|---|---|
| openai | `gpt-image-2` | | 旗舰、引入推理（2026-04-21 GA） |
| openai | `gpt-image-1.5` | ✅（由 `gpt-image-1` 改此） | 比 1 快 ~4×、便宜 ~20%（2025-12-16） |
| gemini | `gemini-3-pro-image` | | Nano Banana Pro，2K–4K + 高保真文字 |
| gemini | `gemini-3.1-flash-image` | ✅（由 `gemini-2.5-flash-image` 改此） | Nano Banana 2，flash 速度 pro 质量（2026-02-26 GA） |
...
> Gemini 的 `-preview` 变体 **2026-06-25 弃用**——目录只收 GA id。
...
2. `BrandAssetRecord` / `SaveBrandAssetInput` 加 `surface?: "android" | "ios"`、`variant?: string`（§5.2）。
3. `assertValidSource`：`app-icon` 用 `image_ref`；`store-shot` / `banner` / `poster` 用 `html`。
4. 新增 per-kind 的 `surface`/`variant` 校验：app-icon/poster 的 `variant` 必填且在合法集内；store-shot/banner 不接受 `variant`。
...
- `save_brand_asset` schema 加 `banner` kind、`surface?`、`variant?`；`target` 去掉 `{ preset }`；**app-icon 的 `source` 由单 `image_ref` 改为 `{ logo_ref, bg_ref, safe_logo_ref }` + 可选 `colors`，一次调用产出整套变体（§6.5）**。
- 工具描述同步。
...
### 6.4 四类资产分辨率标准（core 内表，附 provenance）

落地前逐项对平台官方文档核实并记录 `source` + `verifiedAt`（沿用 `STORE_SHOT_PRESETS` 既有 provenance 纪律）。
...
| platform | surface | variant | 尺寸集 | alpha |
|---|---|---|---|---|
...
> 本表是**最终输出规格**；产出方式见 §6.5（mobile/tablet 3 张基图、web/desktop 2 张基图 + sharp 派生，非每变体生图）。web/desktop 的 standard **加平台圆角**（D17），mobile/tablet 方形不加圆角（系统遮罩）。
>
> 笔误修正：Android 前景安全区"66×66"应为 ~660px（108dp 画布的 66dp 安全区，1080 画布等比，即基图 c 的安全区）——写入 craft 脚手架而非 core；iOS "024×1024" 取 1024×1024；桌面 "720×72" / "358×358" 为笔误/非标，按标准 .ico/.icns 集落地（已剔除）。

### 6.5 应用图标多版本重构（D7 + D16）

产出方式：AI 只生成 **3 张 1080×1080 基图**，其余变体由 core 用 sharp **合成 / tint / 缩放**派生（零额外生图，确定性、无网络）。
...
- **c** = LOGO 缩于中央 **666** 安全区 + 透明背景（alpha；供 Android 自适应前景与 iOS 着色，留遮罩/缩放余量）— **仅 mobile/tablet**
...
**接口变化**：`save_brand_asset(kind="app-icon")` 的 `source` 由单 `image_ref` 改为 `{ logo_ref, bg_ref, safe_logo_ref }` + 可选 `colors`；**一次调用产出该平台整套变体记录**（每 surface×variant 一条 manifest 记录，各带其尺寸文件），并**整组替换**既有 app-icon 记录（图标天然无孤儿，不依赖 D15）；返回 `{ assets: [...] }`。`resolveBrandImageRef` 按 §5.3 取 standard。
...
- 商店图：对每个 `store_shots` 条目（android/ios 或单），按 `count` 循环生图 + 排版 HTML + `save_brand_asset(kind="store-shot", surface?, name="<surface>-<n>", target={width,height})`。
- 横幅：`banners` 非空时，每 surface 一张，背景素材复用 `generate_image(purpose="hero")`；HTML 守中央安全区、不放设备外壳、高对比；`save_brand_asset(kind="banner", surface?, name)`。
- 海报：对每个开启的 `posters` 样式各一张，`save_brand_asset(kind="poster", variant=<style>, name=<style>, target)`。
- 重跑确认（D12）：先列已有资产、明确"将重新生成/覆盖 X"，确认后再执行。

**fm-app-icon**（×3）：

- 前置：调 `get_brand_asset_plan` 取 `app_icon` 段——含需生成的基图清单（mobile/tablet=`a/b/c`、web/desktop=`a/b`）与待产出变体集。
- 生成基图（mobile/tablet **3 张** a/b/c；web/desktop **2 张** a/b），各 `generate_image(purpose="app-icon", count=3)` → Read 视检选优。
- 读 brand tokens（`get_style`）取 tint/单色/深背景色。
- **一次** `save_brand_asset(kind="app-icon", source={ logo_ref, bg_ref, safe_logo_ref }, colors?)`——core 派生并整组替换该平台全套变体。
...
- 图标 **3 基图脚手架**（其余变体由 core 派生，无需脚手架）：`app-icon-logo`（透明背景 LOGO，单一主体、主体占比 ~60–80%、无文字）、`app-icon-background`（纯色/渐变/图案、不透明、无前景元素）、`app-icon-safe-logo`（LOGO 缩于中央 **666/1080** 安全区 + 透明背景）。各含无文字 / 单主体 / 可缩至 ~24px 的否决项。
...
`packages/agent/templates/{claude,codex,gemini}/` 当前只有 5 个命令（design / refine-components / change-style / brand-assets / app-icon），**源码模板里已无 `fm-rollback-design`**。用户仍能看到它是因为残留在已安装目录（`~/.forma/backups/...`、`~/.claude/commands/fm-rollback-design.md` 等历史安装产物）——**重装一次 agent 命令即清除**。
...
即：**均为全量重生成**（非增量）。design/components 留历史版本，app-icon/brand-assets 按名覆盖。**决策**：维持该语义，仅在每次重跑前增加一道"将重新生成/覆盖 X"的明确确认（D12）。
...
- 新增/重写：`store-shot-presets.test.ts` → `brand-asset-plan.test.ts`（计划派生：各 platform × 设置组合）；`brand-assets.test.ts`（banner kind、surface/variant、app-icon 整组替换、alpha 保留、`delete_brand_asset` 清理）；**`icon-derive.test.ts`（合成 / alpha 掩膜填色 / 去色 / 缩放派生，确定性、无 stub）**；`image-models.test.ts`（4 新模型 + 尺寸 + provenance）；mcp `tools.test.ts`（`get_brand_asset_plan`、`delete_brand_asset`、banner、surface/variant schema）；product schema 测试（`brand_assets` 校验 3–8 边界）；server 路由测试（`brand-asset-settings` PUT）；web `BrandAssets.test.tsx`（surface 分组、banner）、`ProductDetail.test.tsx`（设置面板）、`api.test.ts`。
...
| 项 | 说明 | Owner |
|---|---|---|
| 4 个新模型精确尺寸表 | 落地时逐一对官方文档核实并补 provenance（候选值见 §6.1） | 实现期（先行项） |
| 四类资产分辨率 provenance | §6.4 各值落地时记录 `source` + `verifiedAt` | 实现期 |
...
|---|---|---|---|
| `gpt-image-2` | GA 2026-04-21（snapshot `gpt-image-2-2026-04-21`） | 引入推理、image-to-image、强指令遵循；经 Image API / Responses API | https://developers.openai.com/api/docs/models/gpt-image-2 |
| `gpt-image-1.5` | 2025-12-16 | 比 1 快 ~4×、便宜 ~20%；输出至 1536×1024 / 1024×1536；内建于 GPT-5 架构 | https://platform.openai.com/docs/models/gpt-image-1.5 |
| `gemini-3-pro-image` | GA（Nano Banana Pro） | 专业资产、2K–4K、高保真文字、Google Search grounding | https://ai.google.dev/gemini-api/docs/models/gemini-3-pro-image |
| `gemini-3.1-flash-image` | GA 2026-02-26（Nano Banana 2） | flash 速度 pro 质量、高吞吐 | https://ai.google.dev/gemini-api/docs/image-generation |

> 注：Gemini `gemini-3.1-flash-image-preview` / `gemini-3-pro-image-preview` 于 2026-06-25 弃用——目录只收 GA id。OpenAI/Gemini 官方文档正文可能由站点 JS 渲染；实现前必须重开官方页核对 model id、端点、尺寸档与返回字段，若与本文不一致以官方为准并同步目录与测试。
...
| agent 命令（×3 平台） | `packages/agent/templates/{claude,codex,gemini}/fm-app-icon.*`、`fm-brand-assets.*` |
