...
3. `components_initialized` 在 v6 运行时 schema 中无效；当前运行时不会自动改写旧 `product.yaml`。
4. `complete_product_init` 在 v6 删除。产品组件初始化流程成功 commit 后，组件库初始化已经完成，不允许再通过独立工具标记完成。

从 2026-06-05 removal spec 起，旧兼容迁移子系统已删除。当前 v6 运行时直接以 strict schema 和 read model 启动，旧字段或缺失必填契约都不能通过启动校验：

1. `forma serve`、MCP startup、Web startup 和 `forma status` 都不改写运行时 YAML。
2. `product.yaml.components_initialized`、`requirement.yaml.pages[].design_id`、旧页面级 `design_metadata`、页面级 `pen_path` 和页面级 preview 派生字段都是无效 v6 数据。
3. Requirement page 与 baseline page 缺少 `semantic_contract` 时是 schema validation error，不由运行时推断或补写。
4. 页面级 `D-*` 目录不参与 v6 运行时读取、导入、恢复或回滚。已有完整主画布只能由用户显式放入 requirement 目录的 `design.pen` 后再索引。
5. 严格校验失败时，启动或工具初始化必须直接失败并暴露 validation error；不进入只读或部分工具模式。
6. `fm-status` 和 `GET /api/status` 只报告当前可读状态，不修复 YAML、不恢复文件、不启动兼容流程。

strict schema 的实现落点固定如下：

1. `packages/core/src/store.ts` 的 store factory 为 async：`createFormaStore(options)` 在构造 `ProductService`、`RequirementService`、v6 design services 和 `SyncService` 时只接受已经满足 v6 schema 的数据。
2. async store 切换必须和所有调用点迁移在同一个变更集中完成，不能把“store 已变 async、调用点仍按同步对象使用”的状态作为可运行中间版本。`packages/server/src/app.ts` 的 `buildServer(options)` 必须改为 `async`。正常模式下它 `await createFormaStore(...)` 后再 `registerRoutes`。`packages/server/src/index.ts` 的 `main()` 必须 `await buildServer(options)` 后再 `listen`；server 测试、CLI 测试和脚本测试必须统一 `await buildServer(...)` 或传入已经构造好的 async store。`packages/server/src/routes.ts` 的 `FormaStore` 类型必须同步改为 `Awaited<ReturnType<typeof createFormaStore>> & { sync: StoreSync }`，不得继续使用裸 `ReturnType<typeof createFormaStore>`，否则类型会退化成 `Promise<Store>` 并掩盖未 `await` 的调用点。
3. `packages/mcp/src/index.ts` 的 `createFormaMcpServer`、`packages/server/src/app.ts` / `packages/server/src/index.ts` 的 `forma serve` 启动链路、`packages/cli/src/index.ts` 的所有会创建 store 的命令，以及 `scripts/live-style-sync.ts`、`scripts/smoke-pencil.ts` 等 workspace 脚本中所有 `createFormaStore(...)` 调用，都必须 await async store factory 后再注册正常 MCP tools、Fastify routes 或启动脚本逻辑。
4. ProductService、RequirementService、v6 design services、route handler 和 MCP handler 不允许 raw-read 旧 schema 或半修复状态。
5. 已持久化的 `semantic_contract` 只能来自 requirement 保存流程或明确的用户数据修复；运行时不会从 `copy-translations.yaml`、`features`、`fields`、`interactions` 自由文本推断动作、字段或业务能力。
6. 如果已有最小契约无法覆盖真实交互，后续 active session 的 strict Semantic Scope Guard 会按 `REQUIREMENT_UPDATE_REQUIRED` 或 `DESIGN_SCOPE_VIOLATION` 阻断新增语义；v6 strict schema 不能因为缺少旧语义而继续接受缺失 `semantic_contract` 的页面。
...
1. `design.yaml`、`components.yaml`、`manifest.yaml`、`commit-journal.yaml`、`index-journal.yaml` 和 deletion journal 中的正式资源路径必须保存为 `$FORMA_HOME` 相对路径，例如 `data/P-907011/R-c9b123bf/design.pen`、`library/P-907011.versions/3.lib.pen`。字段名优先使用 `*_file`、`*_dir` 或 `*_relative_path`。
...
`semantic_contract` 由 `RequirementService.saveRequirement` 在 `fm-requirement` 保存需求时生成，是页面语义边界的唯一结构化来源。生成输入只能是用户确认后的 requirement 结构化语义字段、navigation、默认语言 copy 和 product rules 结构化语义字段；不得把 baseline 聚合契约、`copy-translations.yaml` 翻译文本、`features`、`fields`、`interactions` 等自由文本作为机器语义来源。`features`、`fields`、`interactions` 等自由文本只能作为说明展示，不能由 `fm-design` 自行解析成允许动作或允许字段。持久化 `requirement.yaml.pages[]` 必须都有 `semantic_contract`；如果调用方绕过服务写入缺失字段，严格 schema 直接返回 schema validation error。`RequirementService.saveRequirement` 新写入或重写入页面时必须把 `semantic_contract_coverage` 写为 `full`；这里的 `full` 只表示“已按当前结构化输入完整生成”，不表示自由文本中的业务含义都被机器理解。`SEMANTIC_CONTRACT_REQUIRED` 只用于 begin session 前发现当前页面的 `semantic_contract_coverage: minimal` 且用户请求的语义超出最小契约时，提示先运行 `fm-requirement` 重新确认结构化语义。
...
5. 如果剩余 source requirements 为空，baseline page 从 `baseline.yaml.pages[]` 删除；如果只剩 `semantic_contract_coverage: minimal` 的来源，baseline page 保留 minimal contract，但 active session 仍按 requirement 页面的覆盖级别决定是否阻断新增语义。
...
6. 任何外部数据修复为旧页面补写最小契约时，也必须调用同一个 builder 的 minimal mode；不能维护第二套推断规则。
...
5. `design_id` 必须从 v6 schema、保存路径、MCP payload、agent prompt 和后台 UI 中删除。当前运行时不得接受它，也不得用它查找 preview、history 或 rollback。
...
7. `SEMANTIC_CONTRACT_REQUIRED` 不表示 strict schema 可接受缺失字段。它只在页面带有 `semantic_contract_coverage: minimal`，且本次设计请求需要的字段、动作、导航、组件或业务 copy 不在最小契约中时返回。真正缺失 `semantic_contract` 的 YAML 是数据损坏，必须返回 schema validation error 并要求修复数据。
...
旧名称清理的范围必须精确：`README.md`、`docs/AGENT.md`、`docs/MCP.md`、安装后的模板、生成的 help、registry、dispatcher 和 Web route table 中不得把旧名称当作可用入口；历史设计说明、删除说明、changelog entries 和负向测试可以出现旧名称，但只能用于说明“已删除、应返回 404/unknown tool/unknown command”。验收命令必须排除这些允许位置，不能用全仓库裸 `rg "complete_product_init|generate_page_design|design_id"` 作为失败条件。
...
这些 mutation routes 和 MCP tools 使用同一套输入 schema、路径参数禁令、stable error code 和 product/pencil lock 规则。启动现在取决于 strict v6 read-model validation；如果 validation 失败，app/server/MCP 在注册这些 mutation routes 或 tools 前就启动失败。后台 UI 的生成、refine、rebuild、rollback 和 component refresh 按钮必须调用这些 routes 或同名 MCP-backed Web adapter；点击后必须立即获得 `session_id` 或 stable error，并通过 active session routes 轮询状态。后台 UI 不得直接调用 raw Pencil MCP，不得向 mutation payload 传入 `canvas_path`、`staging_path`、`outputDir` 或任何文件路径字段。
...
6. 所有前端类型、测试 fixture 和 API client 中的 `design_id` 必须删除；旧 route 和旧字段名只允许在历史设计说明、删除说明或 404/unknown 负向断言测试中以字符串形式出现。
...
7. 所有新增 Web UI 文案必须进入 `packages/web/src/i18n.ts` 的 `en` / `zh` message map，并通过 `useT()` 读取。不得在 `DesignSceneCanvas`、`DesignView`、`PropertyPanel` 或 session 状态组件中硬编码中文或英文 UI 文案。API error code 保持英文 technical literal，UI 展示文案使用本地化 message key。
...
strict store 之前不再有兼容迁移前置检查。启动模式固定为两类：

1. `normal`：`createFormaStore` 通过 strict v6 read-model validation，注册完整 API、MCP-backed Web routes 和后台恢复任务。
2. `fatal_startup_error`：Fastify 自身、端口监听、静态资源目录越界、route 注册失败或 strict read-model validation 失败。这类错误会让 `forma serve` 启动失败，并通过日志或调用方错误暴露原因。
...
4. 启动恢复不读取旧 `requirement.yaml.pages[].design_id`，也不根据页面级 `D-*` 推断状态。strict schema validation 在恢复任务前完成；旧字段仍存在时启动失败，不进入后台恢复队列。
...
9. 目标页面缺少 `semantic_contract`：这只能发生在绕过服务或手工写坏 YAML 的情况下，读取 requirement 时返回 schema validation error 并要求修复数据，不启动设计 session。页面如果只有 `semantic_contract_coverage: minimal` 且用户请求超出契约，才返回 `SEMANTIC_CONTRACT_REQUIRED`，提示先运行 `fm-requirement`。
...
| `DESIGN_COMMIT_RECOVERY_REQUIRED`、`LOCK_CORRUPT`、`PREVIEW_NOT_EXPORTED`、`DESIGN_SESSION_AUDIT_LINK_MISSING` | 409 | `isError: true` | 本地状态需要恢复、lock 不可信或数据完整性阻断对应设计 API |
...
13. `components_initialized` 不再写入 `product.yaml`；组件库初始化状态由 `components.yaml.current_version` 和版本快照派生，`complete_product_init` 不在 MCP registry、dispatcher、agent 模板或用户可见运行文档中出现；只允许在历史设计说明、删除说明和负向测试中作为“已删除入口”出现。
...
45. 新增后台设计页、session 状态、图谱和相关 API error 文案都进入 `packages/web/src/i18n.ts` 的 `en` / `zh` message map，并通过 `useT()` 读取；API error code 只作为 technical literal 展示，不替代本地化说明。
...
53. v6 strict schema 不接受 `product.yaml.components_initialized`、`requirement.yaml.pages[].design_id`、页面级设计派生字段或缺失的 `semantic_contract`；当前运行时不会自动补写或删除这些字段。
...
56. 旧 MCP 工具名和被删除的旧 agent route 名（例如 `fm-refine-design`）不在 registry、dispatcher、installer manifest 或用户可见运行文档中作为可用入口出现；历史设计说明、删除说明和负向测试可以引用旧名称，但必须明确它们应返回 unknown tool/unknown command 或默认 404。显式调用这些旧名称时按平台默认 unknown tool/unknown command 处理，不进入 Forma handler。`fm-rollback-design` 是保留并改造的 v6 route，不按旧 route 删除。

57. v6 strict schema 下缺少 `semantic_contract` 的页面不能被读取为有效 requirement，必须返回 schema validation error 并要求先修复数据；`semantic_contract_coverage: minimal` 且用户请求超出契约时，`fm-design` 返回 `SEMANTIC_CONTRACT_REQUIRED`，要求先运行 `fm-requirement`。
...
59. 旧兼容迁移状态读取和写回入口已删除。`fm-status` 和 `GET /api/status` 不写 YAML、不恢复文件、不提供兼容迁移状态。

60. `forma serve` 启动时直接创建 strict store；v6 read-model validation 失败时启动失败并暴露 validation error，不注册产品、需求、设计或组件 mutation routes。
...
v6 不内置回滚到页面级 `D-*` 模型的旧模型写入，也不提供自动恢复旧 YAML 的兼容入口。当前策略固定如下：

1. v6 runtime 只读取满足 strict schema 的当前数据；旧数据修复必须在运行时外完成。
2. requirement-level `design.pen`、产品级组件库 latest 和版本快照作为 v6 设计资产保留；旧版本服务不读取这些 v6 路径，也不承诺把它们转换成 v5 页面级 `D-*` 输出。
3. 如需回到旧版本服务，操作者必须使用自己维护的外部备份和回退流程；Forma 当前运行时不提供自动数据回滚命令。
...
| `product.yaml.components_initialized` 仍在 schema、创建产品、配置检查、Web 类型和 UI 中使用 | `packages/core/src/product.ts`、`packages/web/src/api.ts`、`packages/web/src/pages/ProductList.tsx`、`packages/web/src/pages/ProductDetail.tsx`、agent shared templates | 第 5 步组件库状态全部切到 `components.yaml` 和版本快照；strict schema 直接拒绝旧字段 |
| 页面级 `design_id` 仍在 requirement page schema、DesignService、server routes、Web API、Web 页面和 tests 中使用 | `packages/core/src/requirement.ts`、`packages/core/src/design.ts`、`packages/server/src/routes.ts`、`packages/web/src/api.ts`、`packages/web/src/pages/RequirementDetail.tsx`、相关 tests | 第 4 步删除 public 旧入口；第 5、7、10 步用 requirement-level `design.yaml`、preview index 和新 routes 替换；strict schema 直接拒绝旧字段 |
...
| README、docs 和 agent shared guidance 仍把旧工具描述为可用入口 | `README.md`、`docs/AGENT.md`、`docs/MCP.md`、`packages/agent/templates/shared/SKILL.md` | 第 4 步和 public legacy surface removal 同步更新；旧名称只保留在历史说明、删除说明和负向测试 |
...
| Pencil lock 路径和语义仍是旧 `$FORMA_HOME/pencil.lock` 长生命周期锁 | `packages/core/src/paths.ts`、`packages/core/src/pencil.ts`、`packages/core/tests/pencil.test.ts`、`packages/core/tests/foundation.test.ts` | 第 6 步迁移到 `$FORMA_HOME/locks/pencil.lock` 短事务锁，并删除旧 lock 兼容检测 |
...
v6 必须按以下顺序落地，不能先暴露新入口再保留旧模型并行运行。每一步都必须能独立通过测试。

1. Legacy compatibility preparation：已由 2026-06-05 removal spec 废止。当前实现不新增数据改写、备份或兼容状态读取基础设施。
2. Async startup skeleton：把 `createFormaStore`、`buildServer`、MCP/server/CLI/script/test 入口改为 async。普通启动直接创建 strict store；validation 失败即启动失败。验收：`rg "createFormaStore\\(" packages scripts` 中所有运行调用点都有 `await` 或返回 promise 链，Server/MCP validation-failure startup tests 通过，现有运行模型 tests 通过。
3. Legacy data rewrite stage：已由 2026-06-05 removal spec 废止。当前运行时不提供 CLI/API 数据改写或恢复入口。
4. Public legacy surface removal：在启用 v6 strict schema 前，先从 MCP registry、agent route list、installer manifest、server route table、SPA route table、README/docs/agent templates 和 Web API client 删除旧 public 入口：`complete_product_init`、旧一次性 `generate_components` 写入 handler、`generate_page_design`、`save_designs`、`generate_and_save_page_design`、`rollback_design`、`diff_designs`、`get_design_annotations`、`export_design_asset`、`fm-refine-design`、`/api/designs/:designId/*` 和 `/products/:productId/requirements/:requirementId/designs/:designId`。内部旧 `DesignService` 可以暂时保留为未导出的私有测试 helper，但任何 public call 必须 unknown tool/default 404。验收：负向 public surface tests 通过，docs/help/install 输出不再暴露旧入口。
5. v6 strict schema 与 core read model：启用 v6 strict schema，删除运行时代码对 `components_initialized`、`design_id`、页面级设计派生字段和 deterministic `D-*` preview fallback 的依赖；新增 requirement-level `design.yaml`、产品级 `components.yaml`、baseline image requirement-level lookup、产品删除 v6 component library 路径和 active design session 阻断。仍不注册新设计写入工具。验收：旧字段重新出现时 schema validation error；旧页面级 `D-*` 不参与状态读取；baseline image 不扫描旧 preview。
