...
r2p_version: 3
r2p_status: approved
r2p_created_at: 2026-06-02T02:17:32.447331+00:00
r2p_updated_at: 2026-06-02T02:24:59.288459+00:00
---
...
| SPEC-FR-004 [ADDRESSED], SPEC-IF-003 [ADDRESSED], SPEC-IF-004 [ADDRESSED], SPEC-IF-005 [ADDRESSED], SPEC-IF-006 [ADDRESSED], SPEC-DATA-005 [ADDRESSED], SPEC-ERR-004 [ADDRESSED], SPEC-ACC-003 [ADDRESSED], SPEC-ACC-004 [ADDRESSED] | SPEC dev MCP handoff | PLAN-TASK-008 | implementation + MCP tests | covered | archived gate and read-only VZI projection with tokens/annotations |
...
- [ ] red: Add a parse/transform/encode/decode smoke test using a Forma design-page HTML fixture containing text, layout, inline SVG, and at least one resource; assert decoded non-zero bounds, non-empty color/font tokens, and generated annotations (spacing/alignment/dimension).
- [ ] red: Add tests that VZI/icon mapping resolves to generated icon files and mismatch fails before archive commit.
- [ ] green: Implement `captureRequirementVzi(deps, input, iconExportResult)` using the vendored Puppeteer parser, transformer, and encoder; invoke the transformer with annotation generation enabled (`enableAnnotations: true`) so the VZI carries color/font tokens and spacing/alignment/dimension annotations.
...
it("get_page_ui returns resolved icon asset refs for archived VZI data", async () => {
  ...
  expect(result.tokens).toMatchObject({ colors: expect.any(Array), fonts: expect.any(Array) });
  expect(Array.isArray(result.annotations)).toBe(true);
});
```

Steps:
- [ ] red: Add MCP tests for non-archived rejection, archived handoff success, missing generated files, page UI depth/fields/node root behavior, top-level de-duplicated color/font tokens and tree-scoped annotations in `get_page_ui`, node detail with node-scoped annotations, search results, and resolved icon `assetRef`.
...
- [ ] green: Decode VZI through the vendored read layer; project `get_page_ui` as page viewport/platform metadata, top-level de-duplicated color/font tokens, a depth-limited tree, and tree-scoped annotations, and project node/search JSON, all with absolute same-machine asset paths inside artifact/export roots.
