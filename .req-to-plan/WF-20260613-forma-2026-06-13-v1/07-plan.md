...
r2p_version: 2
r2p_status: approved
r2p_created_at: 2026-06-12T19:37:34.640304+00:00
r2p_updated_at: 2026-06-12T19:52:39.484955+00:00
---
...
范围承接：SCOPE-IN-002, SCOPE-IN-008
...
- packages/server/src/app.ts
- packages/server/tests/routes.test.ts
- packages/od-diagnostics/src/redaction.ts
- packages/od-diagnostics/tests/redaction.test.ts
- packages/od-diagnostics/tests/zip.test.ts
Skeleton:
```ts
...
- [ ] 凭证排除测试：静态服务与既有导出端点均取不到 media-config.yaml 内容；错误响应 details 不含 key；server audit/request 日志不记录 key、base_url 查询串或原始 PUT payload
- [ ] 诊断包红线：`packages/od-diagnostics` redaction 识别 `api_key` / `authorization` / `media-config.yaml` 内容；zip 测试确认诊断输出只含脱敏元数据，不含配置文件原文
- [ ] 测试覆盖四端点形态 + 脱敏（env 来源不回尾号）
Verification: npx vitest run packages/server/tests/routes.test.ts packages/od-diagnostics/tests/redaction.test.ts packages/od-diagnostics/tests/zip.test.ts
...
- packages/core/tests/release-packaging.test.ts
Skeleton:
```ts
...
- [ ] 明确 assets 随 core 包发布的方案：将 `lucide-icons.json` 复制进 `dist/assets` 或更新 `packages/core/package.json` `files` 白名单；`release-packaging.test.ts` 断言 pack/build 产物包含该 JSON
- [ ] 测试：命中、空结果、limit；幂等验证在初次生成的 `lucide-icons.json` 已存在后执行（重跑脚本并用 checksum/cmp 确认内容无变化），不要在同一任务的首次生成后用 `git diff --quiet` 当作通过条件
Verification: node scripts/vendor-lucide.mjs && pnpm --filter @xenonbyte/forma-core build && npx vitest run packages/core/tests/icon-search.test.ts packages/core/tests/release-packaging.test.ts；Idempotence: shasum -a 256 packages/core/assets/lucide-icons.json > /tmp/forma-lucide-icons.sha && node scripts/vendor-lucide.mjs && shasum -a 256 -c /tmp/forma-lucide-icons.sha
...
- [ ] `createFormaStore` 暴露 `saveBrandAsset` / `listBrandAssets` / `exportBrandAssetsZip` / `resolveBrandImageRef`，并在 `packages/core/src/index.ts` 导出对应类型与函数；MCP/server 后续任务只调用 store API，不直连内部文件路径
- [ ] zip 导出复用既有 zip 能力；路径全程 path-boundary 校验；导出测试确认绝不包含 `$FORMA_HOME/media-config.yaml`
- [ ] 测试：锁、平台尺寸组、manifest 覆盖语义、zip、brand 引用解析、store/public export 类型可用（管线测试同步补 brand/ 用例）
...
范围承接：SCOPE-IN-004, SCOPE-IN-008
Spec References: SPEC-BEHAVIOR-007, SPEC-BEHAVIOR-006
...
- [ ] 在 renderer 建好后，把 `saveBrandAsset` 的 `store-shot` / `poster` HTML source 接到 `renderBrandAssetHtml`，按 target/preset 得到 PNG 后写入 brand-assets 存储并进入 manifest；core 测试必须从 public save path 验证 HTML→PNG，不只测 renderer helper
- [ ] 拦截层四类测试：脚本拦截、远程请求拒绝、file 越界拒绝、白名单内预览可用
Verification: npx vitest run packages/core/tests/brand-asset-render.test.ts packages/core/tests/brand-assets.test.ts
...
- [ ] 测试：schema 拒绝（source 皆缺/皆给、未知 kind/preset）、`BRAND_ASSET_*` 错误经 `{ error_code, message, details }` 返回、stub 全链路；HTML source 通过 MCP `save_brand_asset` 走 store/render public path 并落 PNG
Verification: npx vitest run packages/mcp/tests/tools.test.ts
...
- [ ] 测试：列表形态、文件服务边界（越界拒绝）、zip 导出、不存在产品/资产 404；zip 响应不含 media-config.yaml 或任意 provider key 内容
Verification: npx vitest run packages/server/tests/routes.test.ts
...
范围承接：SCOPE-IN-004, SCOPE-IN-007
...
范围承接：SCOPE-IN-005, SCOPE-IN-007
...
- [ ] save_brand_asset 的 `target.preset` 接通 preset 表；渲染尺寸测试断言像素精确；`createFormaStore` 与 `packages/core/src/index.ts` 导出 `StoreShotPreset` 与 `listStoreShotPresets`
Verification: npx vitest run packages/core/tests/store-shot-presets.test.ts packages/core/tests/brand-assets.test.ts

### PLAN-TASK-025 M5：MCP preset 查询 + fm-brand-assets 新命令模板（×3 平台）
范围承接：SCOPE-IN-006, SCOPE-IN-007
...
Skeleton:
```md
MCP: list_store_shot_presets(product_id) -> { presets: [{ id, width, height, source, verified_at }] }
...
guide to fm-design) -> list_store_shot_presets -> read brand tokens + app icon + design previews ->
...
```
Steps:
- [ ] MCP `list_store_shot_presets(product_id)` 工具读取产品 platform 后返回 core preset 表（id/width/height/source/verified_at）；测试覆盖 platform 过滤与无产品/无 preset 错误
- [ ] 三平台模板编写；前置检测按 5.2 矩阵全集（含 ICON 硬前置与商店图的预览硬前置）
- [ ] fm-brand-assets 模板必须先调用 `list_store_shot_presets`，再选择返回的 preset id 传给 `save_brand_asset(target: { preset })`；禁止硬编码未查询的 preset 名称
- [ ] template-parity 断言把新命令纳入全集
- [ ] 画布分组验证：商店图/海报组随数据出现（PLAN-TASK-019 的动态分组，无代码改动）
- [ ] M5 手动验收（用户）：真实 key 全流程出一套商店图 + 海报并 zip 导出
Verification: npx vitest run packages/mcp/tests/tools.test.ts packages/agent/tests/template-parity.test.ts；M5 收口：pnpm test && pnpm typecheck，绿后请用户运行 /check
...
list_store_shot_presets(product_id)                                 → 返回当前产品 platform 可用 preset
```

**server 路由（新增 7 端点：M1 四个 media 方法/路径 + M3 三个 brand-assets 方法/路径）**：
...
| M1 | server 路由测试 + web Settings 测试 | M1 四个 media 方法/路径；配置节交互（厂商→模型联动、`••••`+尾 4 位） |
| M1 | 凭证排除测试 | 静态服务/既有 zip 导出/诊断不含 media-config.yaml 内容；server 日志与错误 details 不含 key |
| M2 | 图标检索测试、release-packaging 测试、`vendor-lucide.mjs` checksum/cmp 幂等测试、mcp `search_icons` 测试、模板断言 | 命中/空结果/limit；icons.json 初次生成后重跑脚本内容不变；build/pack 产物包含 lucide-icons.json；模板含禁手绘硬规则 |
| M3 | `packages/core/tests/brand-assets.test.ts` | 锁内原子写、平台尺寸派生组、manifest 记账、store/public exports、zip 导出、覆盖语义，且 zip 不含 media-config.yaml |
| M3 | 渲染沙箱测试 | 脚本拦截、远程请求拒绝、file 越界拒绝、白名单预览可用；HTML source 通过 saveBrandAsset public path 调 renderer 并落 PNG |
...
| M5 | preset 表测试、MCP preset 查询、渲染尺寸测试、模板断言 | preset 表每条含来源 URL + 核实日期；agent 通过 `list_store_shot_presets` 发现 preset；按 preset 渲染输出像素精确 |
...
2. 五个 MCP 工具的最终入参/返回 schema（`generate_image` / `search_icons` / `save_brand_asset` / `list_brand_assets` / `list_store_shot_presets`，原始文档 §6 已给草案并由 PLAN 补齐 preset 查询）。
3. server 七条新路由的方法/路径/响应形态（M1 四个 media 方法/路径 + M3 三个 brand-assets 方法/路径）。
