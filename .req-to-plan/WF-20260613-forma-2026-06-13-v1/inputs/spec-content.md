...
**错误码（6 新增）**：`MEDIA_NOT_CONFIGURED` / `MEDIA_PROVIDER_ERROR` / `MEDIA_INVALID_INPUT` / `MEDIA_IMAGE_NOT_FOUND` / `BRAND_ASSET_INVALID_INPUT` / `BRAND_ASSET_NOT_FOUND`，语义见各行为契约；HTTP 映射沿用 FormaError 既有机制（INVALID→400、NOT_FOUND→404、NOT_CONFIGURED→409 或 422 取既有惯例、PROVIDER_ERROR→502）。
...
- 火山方舟（2026-06-13 已核）：模型列表 82379/1330310（页面更新 2026.06.12）、图片生成 API 82379/1541523（2026.06.04）、Seedream 教程 82379/1824121（2026.06.04）。**M1 实现前必须重新打开复核** model id / endpoint / `response_format` / 返回字段 / size 映射，不一致以官方为准并同步目录与测试。
- App Store Connect / Google Play / Open Graph 商店图规格：**尚未核**，M5 实现前核定（当前所有像素示例值 UNCONFIRMED，禁止落表/落测试）。
- lucide-static：M2 实现时锁定版本号并随 `vendor-lucide.mjs` 产物记录；ISC 许可证随附。
...
| Test Matrix M3 模板断言 | RISK-DATA-001（旧 artifact 零迁移回归） | covered |
