...
Options:
  尺寸: A) Android 前景安全区按 D16 用 666×666（66×66 系笔误）；桌面图标集用标准档 {1024,512,256,128,64,32,16}，丢弃可疑的 358×358 与 720×72。
        B) 严格保留源文档所有值（含 66×66 / 358×358 / 720×72）。
  海报默认: A) 三个海报开关默认全开（portrait+landscape+square）。
            B) 默认仅竖屏开，横屏/方形默认关。
            C) 默认全关（用户按需开）。
Recommended: 尺寸 A + 海报默认 A（最贴合平台规范且开箱即用；最终像素值仍以实现期官方核实为准）。
Status: pending
```
```text
...
Options: A) 仅当实现期核实兼容后才把 provider 默认切到 gemini-3.1-flash-image；不兼容则目录登记新模型但默认仍保 gemini-2.5-flash-image，原生 generateContent renderer 单列后续任务（不扩本批范围）。
         B) 本批直接实现原生 generateContent renderer 以保证默认切换（范围扩张，OVERFLOW-001）。
Recommended: A（不阻塞其余 9 项，避免范围扩张）。
Status: pending
```
...
| DECISION-001 | RISK-CORR-003, OQ-005/006, D6 | pending |
| DECISION-002 | RISK-DEP-001, ASSUME-005, OQ-004 | pending |
