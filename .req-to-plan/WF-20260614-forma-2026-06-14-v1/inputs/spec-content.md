...
### SPEC-BEHAVIOR-001 生图模型目录与默认（DES-ARCH-001）
...
### SPEC-BEHAVIOR-002 platform→surface 映射（DES-ARCH-002）
...
### SPEC-BEHAVIOR-003 资产计划 getBrandAssetPlan（DES-ARCH-005）
...
### SPEC-BEHAVIOR-004 应用图标本地派生（DES-ARCH-007）
...
### SPEC-BEHAVIOR-005 save_brand_asset(app-icon) 判别联合 + 原子替换（DES-ARCH-008）
...
### SPEC-BEHAVIOR-006 delete_brand_asset（DES-ARCH-008）
...
### SPEC-BEHAVIOR-007 破坏式移除 + 引用清零（DES-ARCH-005/009，含审计补全）
...
### SPEC-BEHAVIOR-008 web 设置 + 画布 surface 分组（DES-ARCH-010/011）
...
### SPEC-BEHAVIOR-009 agent 模板 + craft + CLAUDE.md（DES-ARCH-012）
...
// productSchema (.strict() chain): + brand_assets: brandAssetsSettingsSchema.optional()
// productConfigSchema (separate, 后台写入用): 同字段 optional
```
...
type SavedAppIconResult = { assets: BrandAssetRecord[] };
```
MCP `save_brand_asset` 的 zod source-refine（现 `image_ref XOR html` + `app-icon→image_ref`）整体替换为上述判别联合。
...
- `packages/core/src/store.ts`：`listStoreShotPresets` 转发 + `type StoreShotPreset` import（:18）共 ~4 行。
...
- 测试：删除 `store-shot-presets.test.ts`（整文件）；`tools.test.ts` 的 `:6668-6790` describe 块 + `:277` mock + 相关断言。
...
- **sharp 0.34.5（已核实，审计 subagent）**：`greyscale`/`tint`/`composite{blend:"dest-in"}`/alpha 保留/`resize`/SVG mask 光栅化均支持 → SPEC-BEHAVIOR-004 可实现，无版本缺口。
- **新模型 model id / 允许尺寸 / Gemini renderer 兼容性（UNCONFIRMED，实现期核实）**：本阶段未对 OpenAI/Gemini 官方文档做 Context7/官方核实；按 ASSUME-004/RISK-DEP-002 推迟到实现期，核实后写入 catalogue 并附 source+verifiedAt；未核实值不进断言；Gemini 默认切换受 DECISION-002 A 门控。
- **平台商店图/横幅/图标推荐分辨率（UNCONFIRMED，实现期核实）**：沿用 `STORE_SHOT_PRESETS` 既有"官方来源 URL + verifiedAt"约定逐项核实；结构先行、数值后填。
- **poster 三样式 / 桌面图标档 / Android 安全区（已定，DECISION-001 A）**：受控常量，可直接断言。
...
- **TM-07 save(app-icon)**：判别联合输入派生整套并原子替换、返回 {assets}；重复保存替换不累积。
