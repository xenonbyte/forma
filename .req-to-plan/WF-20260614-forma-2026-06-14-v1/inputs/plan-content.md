...
### PLAN-TASK-001 核心类型 + product 设置 schema + surface 映射
Spec References: SPEC-DATA-001, SPEC-DATA-002, SPEC-BEHAVIOR-002
Change Type: modify
TDD Applicable: yes
Files:
...
export function brandSurfacesForPlatform(platform: Platform): BrandSurface[] {
  return platform === "mobile" || platform === "tablet" ? ["android", "ios"] : [];
}
// brand-assets.ts
export const BRAND_ASSET_KINDS = ["app-icon", "store-shot", "banner", "poster"] as const;
const KIND_SUBDIR = { "app-icon": "app-icon", "store-shot": "store-shots", banner: "banners", poster: "posters" } as const;
export interface BrandAssetRecord { /* ...existing... */ surface?: BrandSurface; variant?: string; }
// brandAssetRecordSchema: + surface: z.enum(["android","ios"]).optional(), variant: z.string().min(1).optional()
// product.ts — 字段加进现有 base.extend({...}) 对象内（在 .strict() 之前），不参与 superRefine
const brandAssetsSettingsSchema = z.object({
...
// productSchema/productConfigSchema 对象内: brand_assets: brandAssetsSettingsSchema.optional()
// ProductService.updateBrandAssetSettings(productId, patch) 经 runProductMutation
```
Steps:
- [ ] (SCOPE-IN-006) 加 `BrandSurface` + `brandSurfacesForPlatform`（schemas.ts），覆盖四端单测
- [ ] (SCOPE-IN-003) `BRAND_ASSET_KINDS` 增 `banner`、`KIND_SUBDIR` 增 `banner`，`brandAssetRecordSchema` 增 surface/variant（optional）
- [ ] `BrandAssetRecord` 加 `surface?`/`variant?`
- [ ] product schema 加 `brand_assets` 块（注意加进 `.extend({...})` 对象内，DECISION-001：3 poster 默认 true）+ `updateBrandAssetSettings`（经锁）
- [ ] 更新失配的既有 fixtures（RISK-DATA-003：更新而非迁移）
Verification: npx vitest run packages/core/tests/brand-assets.test.ts packages/core/tests/product-config.test.ts; pnpm typecheck

### PLAN-TASK-002 getBrandAssetPlan + 计划类型 + 分辨率表
Spec References: SPEC-BEHAVIOR-003, SPEC-DATA-003, SPEC-DATA-004
Change Type: create
TDD Applicable: yes
Files:
...
export interface BrandAssetPlanEntry {
  kind: BrandAssetKind; surface?: BrandSurface; variant?: string;
  width: number; height: number; count: number;
  source?: string; verifiedAt?: string;
  baseImages?: ("a" | "b" | "c")[]; variants?: string[];
}
export interface BrandAssetPlan { productId: string; platform: Platform; surfaces: BrandSurface[]; entries: BrandAssetPlanEntry[]; }
// 受控常量（DECISION-001，可断言）
const POSTER_SIZES = { portrait: [1080,1920], landscape: [1920,1080], square: [1080,1080] } as const;
const DESKTOP_ICON_SIZES = [1024,512,256,128,64,32,16] as const;
const ANDROID_SAFE_AREA = 666;
// 平台尺寸表：键 (platform[,surface])，值 {width,height,source,verifiedAt}；平台像素值 UNCONFIRMED 占位待核实
export function getBrandAssetPlan(product: Product): BrandAssetPlan { /* surfaces=brandSurfacesForPlatform; store-shot count=store_shot_count/surface; banner if on; poster per 3 switches; app-icon per surface {baseImages, variants} */ }
```
Steps:
- [ ] (SCOPE-IN-002) store-shot：每 surface 1 条 `count=store_shot_count`（单 surface 端无 surface 槽）
...
### PLAN-TASK-003 应用图标 sharp 本地派生
Spec References: SPEC-BEHAVIOR-004, SPEC-DATA-005
Change Type: create
TDD Applicable: yes
Files:
...
export interface DeriveIconInput {
  surface?: BrandSurface; platform: Platform;
  logo: Buffer; background: Buffer; safeLogo?: Buffer;
  colors?: { mono?: string; tint?: string; dark_bg?: string };
}
export interface DerivedIconVariant { variant: string; width: number; height: number; png: Buffer; }
export async function deriveAppIconVariants(input: DeriveIconInput): Promise<DerivedIconVariant[]> {
  // 归一化母图到 1080² 工作画布；按 surface 派生：
  // android: android-standard(512,a∘b) / -foreground(1080,c) / -background(1080,b) / -monochrome(1080,a→greyscale+tint黑+alpha)
  // ios: ios-standard(1024/180/120,a∘b 缩放) / ios-dark(1024,a+tint+深色底) / ios-tinted(1024,c+tint+alpha)
  // web/desktop(无 surface): standard 一张最大圆角图（SVG rect rx mask + composite blend:"dest-in"），其余档 resize
}
```
Steps:
...
### PLAN-TASK-004 save_brand_asset 判别联合 + 原子替换 + delete_brand_asset + bare-ref 规则
Spec References: SPEC-BEHAVIOR-005, SPEC-DATA-006, SPEC-BEHAVIOR-006
Change Type: modify
TDD Applicable: yes
Files:
- packages/core/src/brand-assets.ts
...
type SaveBrandAssetInput =
  | { product_id:string; kind:"app-icon"; brand_style:string; platform:Platform; logo_ref:string; bg_ref:string; safe_logo_ref?:string; colors?:Record<string,string>; model?:string }
  | { product_id:string; kind:"store-shot"|"banner"|"poster"; name:string; brand_style:string; source:{html:string}; surface?:BrandSurface; variant?:string; target:{width:number;height:number}; model?:string };
type SaveBrandAssetResult = { kind:"app-icon"; assets: BrandAssetRecord[] } | { kind:"store-shot"|"banner"|"poster"; asset: SavedBrandAsset };
export async function deleteBrandAsset(deps: BrandAssetDeps, input: { product_id:string; kind:BrandAssetKind; name:string }): Promise<{deleted:boolean}> {
  // 解析记录文件路径；强制落在 brand-assets/<KIND_SUBDIR>/ 内（绝对/.. → FormaError(MEDIA_INVALID_INPUT)）；runProductMutation 删 manifest+文件；不存在→FormaError
}
// resolveBrandImageRef: bare "forma-image://brand/app-icon" → 选 standard 类最大尺寸文件（替换 MASTER_SIZE=2048 依赖）；无匹配→FormaError
```
Steps:
- [ ] (SCOPE-IN-007) app-icon 分支调 `deriveAppIconVariants`，原子替换该 product 全部 app-icon 记录，返回 {assets}
- [ ] store-shot/banner/poster 分支带 surface/variant 入记录、target 取计划尺寸
- [ ] `deleteBrandAsset` + 路径边界 + 锁；`resolveBrandImageRef` 改 standard-largest 规则（移除 name="primary"，不依赖 MASTER_SIZE）
- [ ] 单测：判别联合派生+原子替换+不累积；delete 越界/绝对/`..`/不存在；bare-ref 解析
Verification: npx vitest run packages/core/tests/brand-assets.test.ts; pnpm typecheck

### PLAN-TASK-005 破坏式移除（preset 体系 + name="primary"）
Spec References: SPEC-BEHAVIOR-007, SPEC-DATA-008
Change Type: modify
TDD Applicable: no
Files:
- packages/core/src/brand-assets.ts
...
Skeleton:
```ts
// 删除：STORE_SHOT_PRESETS / PLATFORM_PRESET_MAP / StoreShotPreset / listStoreShotPresets / {preset} target 分支 / name==="primary" 回退(brand-assets.ts:741)
// store.ts: 删 listStoreShotPresets 转发(接口方法:110+impl) + StoreShotPreset value-import(:11) + type-import(:18)
// mcp/tools.ts: 删 list_store_shot_presets @ FORMA_TOOL_NAMES(:69)/schema(:482)/desc(:522)/registration(:668) + save_brand_asset source-refine(:345-380)
// 测试: 删 store-shot-presets.test.ts(整文件); tools.test.ts :6668-6790 块 + :277 mock; brand-assets.test.ts :31 import + :630 用例
```
Steps:
...
Skeleton:
```ts
// IMAGE_MODELS += gpt-image-2, gpt-image-1.5 (openai), gemini-3-pro-image, gemini-3.1-flash-image (gemini)
// openai default → gpt-image-1.5（gpt-image-1 降非默认）
// gemini default 切换条件化：仅核实 OpenAI 兼容 /images/generations 支持后切 gemini-3.1-flash-image；否则保 gemini-2.5-flash-image
// MODEL_SIZE_TABLES += 各新模型 size 表；model id/尺寸/兼容性经官方文档核实后写入 + source/verifiedAt provenance 注释
```
Steps:
- [ ] (SCOPE-IN-001) 官方核实 4 个新模型 id/允许尺寸 + Gemini renderer 兼容性（DECISION-002：核实后再切默认；RISK-DEP-002 deferred 闭合）
- [ ] 写入 catalogue + size 表 + provenance；resolveSize 对新模型不抛
- [ ] 单测：新模型注册、默认指向、resolveSize；未核实像素值不进断言
Verification: npx vitest run packages/core/tests/media/image-models.test.ts; pnpm typecheck

### PLAN-TASK-007 MCP 工具集变更
Spec References: SPEC-DATA-007
Change Type: modify
TDD Applicable: yes
Files:
- packages/mcp/src/tools.ts
...
// + delete_brand_asset(product_id, kind, name) → {deleted}
// save_brand_asset schema → SPEC-DATA-006 判别联合（输入+返回按 kind）
// 新工具不读 media-config；加无凭证字段外泄断言
```
Steps:
- [ ] (SCOPE-IN-007) 注册 get_brand_asset_plan / delete_brand_asset；改 save_brand_asset schema
- [ ] 单测：两新工具注册与返回；save 新 schema；无凭证外泄断言
Verification: npx vitest run packages/mcp/tests/tools.test.ts; pnpm typecheck

### PLAN-TASK-008 后台产品页品牌资产设置 UI + i18n
...
// 保存 → updateBrandAssetSettings（经既有 update_product_config / 新设置端点）
// i18n: en/zh 两块加键（设置标签/选项）
```
Steps:
- [ ] (SCOPE-IN-008) 设置控件 + 保存联动；en/zh 文案齐全
- [ ] 组件单测：交互+保存调用；保存后计划随设置变化
Verification: npx vitest run packages/web/src/pages/ProductDetail.test.tsx; pnpm typecheck
...
Skeleton:
```text
pnpm lint && pnpm typecheck && pnpm test && pnpm check:vzi-boundary
```
Steps:
- [ ] pnpm lint / pnpm typecheck / pnpm test 全绿
- [ ] pnpm check:vzi-boundary 不回退
- [ ] 人工验收（真实 provider 生图）单列，不在自动路径
Verification: pnpm lint; pnpm typecheck; pnpm test; pnpm check:vzi-boundary

## Trace
<!-- Map this stage's IDs to upstream/downstream. R3 derives & checks closure. -->
| This ID | Upstream | Status |
|---|---|---|
| PLAN-TASK-001 | SPEC-DATA-001, SPEC-DATA-002, SPEC-BEHAVIOR-002 | active |
| PLAN-TASK-002 | SPEC-BEHAVIOR-003, SPEC-DATA-003, SPEC-DATA-004 | active |
| PLAN-TASK-003 | SPEC-BEHAVIOR-004, SPEC-DATA-005 | active |
| PLAN-TASK-004 | SPEC-BEHAVIOR-005, SPEC-DATA-006, SPEC-BEHAVIOR-006 | active |
| PLAN-TASK-005 | SPEC-BEHAVIOR-007, SPEC-DATA-008 | active |
| PLAN-TASK-006 | SPEC-BEHAVIOR-001 | active |
| PLAN-TASK-007 | SPEC-DATA-007 | active |
| PLAN-TASK-008 | SPEC-BEHAVIOR-008 | active |
| PLAN-TASK-009 | SPEC-BEHAVIOR-008 | active |
| PLAN-TASK-010 | SPEC-BEHAVIOR-009 | active |
| PLAN-TASK-011 | SPEC-BEHAVIOR-007 | active |
...
### SPEC-BEHAVIOR-001 生图模型目录与默认
`IMAGE_MODELS` 新增 4 条：`gpt-image-2`、`gpt-image-1.5`（provider=openai）、`gemini-3-pro-image`、`gemini-3.1-flash-image`（provider=gemini）。provider 内 `default:true`：openai 迁到 `gpt-image-1.5`（`gpt-image-1` 降为非默认）；gemini 默认切换**条件化**——仅当实现期核实 `gemini-3.1-flash-image` 兼容现有 OpenAI 兼容 `/images/generations` renderer 后才切（DECISION-002 A）；未核实/不兼容则保 `gemini-2.5-flash-image` 为默认、新模型仅登记，并单列"原生 generateContent renderer"后续任务。`MODEL_SIZE_TABLES` 为每个新模型登记 size 表（5 个 aspect）。新模型 model id / 允许尺寸 / 兼容性为 UNCONFIRMED，实现期经官方文档核实后写入 catalogue + provenance 注释（source URL + verifiedAt），未核实值不进断言。`resolveSize(新模型, aspect)` 不抛错。volcengine 仍全局优先（不改全局默认）。

### SPEC-BEHAVIOR-002 platform→surface 映射
纯函数 `brandSurfacesForPlatform(platform: Platform): BrandSurface[]`，`BrandSurface = "android" | "ios"`：`mobile → ["android","ios"]`、`tablet → ["android","ios"]`、`web → []`、`desktop → []`（空数组＝单 surface、记录不写 surface 字段）。唯一真源，被 store-shot/banner/app-icon 计划复用；poster 不经它（平台无关）。

### SPEC-BEHAVIOR-003 资产计划 getBrandAssetPlan
纯函数 `getBrandAssetPlan(product: Product): BrandAssetPlan`。读 `product.platform` + `product.brand_assets` 设置（缺省套用默认）。产出 entries：
- store-shot：对每个 surface（单 surface 端为 1 个无 surface 槽）产 1 条，`count = store_shot_count`。
- banner：`brand_assets.banner === true` 时，对每个 surface 产 1 条 `count:1`；否则 0 条。
- poster：对每个为 true 的开关（portrait/landscape/square）产 1 条，`surface` 省略，`variant` = 样式名，`count:1`。
- app-icon：对每个 surface 产 1 条 `kind:"app-icon"` 计划，附该 surface 需要的母图集（`baseImages`）与派生变体清单（`variants`，见 SPEC-BEHAVIOR-006）。
计划＝目标态，不读磁盘、不含现状；与 manifest 解耦。尺寸取自分辨率表（SPEC-DATA-004）。

### SPEC-BEHAVIOR-004 应用图标本地派生
新增 `brand-icon-derive.ts`：`deriveAppIconVariants(input): DerivedIconSet`。母图归一化到 1080² 工作画布；按 surface 派生：
- **Android**（surface=android）：`android-standard`(512，a 合成 b)、`android-foreground`(1080，=c，透明)、`android-background`(1080，=b，不透明)、`android-monochrome`(1080，a→greyscale+tint(纯黑)+透明)。
- **iOS**（surface=ios）：`ios-standard`（多档，a 合成 b 缩放）、`ios-dark`(1024，a+tint+深色底)、`ios-tinted`(1024，c+tint+透明)。
- **web/desktop**（无 surface）：`standard` 一张最大圆角图（web 512 / desktop 最大档），圆角经 SVG mask + `composite blend:"dest-in"`，其余档由它 resize（D17）。
全程 sharp 确定性算子（0.34.5 已确认支持 greyscale/tint/composite dest-in/alpha 保留/resize/SVG 光栅化）；不逐变体生图。母图集：mobile/tablet 需 a/b/c，web/desktop 需 a/b。

### SPEC-BEHAVIOR-005 save_brand_asset(app-icon) 判别联合 + 原子替换
`save_brand_asset` 按 `kind` 判别：
- `kind:"app-icon"` 输入 `{product_id, kind:"app-icon", brand_style, platform, logo_ref, bg_ref, safe_logo_ref?, colors?, model?}`（不再要 name/surface/variant/target）；core 调 `deriveAppIconVariants` 产出该 product 全部 app-icon 记录，**原子替换**（先删旧 app-icon 记录与文件，再写新集），返回 `{assets: BrandAssetRecord[]}`。
- `kind:"store-shot"|"banner"|"poster"` 保持 `source.html` → PNG 路径，新增 `surface?`/`variant?` 入记录、`target` 取自计划尺寸。
所有写经 `runProductMutation`。

### SPEC-BEHAVIOR-006 delete_brand_asset
`deleteBrandAsset(deps, {product_id, kind, name})`：解析目标记录的文件路径，**强制落在** `data/products/<pid>/od-project/brand-assets/<KIND_SUBDIR>/` 内（绝对/`..`/越界 → `FormaError(MEDIA_INVALID_INPUT)`）；经 `runProductMutation` 删 manifest 记录 + 磁盘文件；记录不存在 → `FormaError`（不静默）。支持降 store_shot_count / 关 poster·banner 后的孤儿清理。

### SPEC-BEHAVIOR-007 破坏式移除 + 引用清零（含审计补全）
删除 `STORE_SHOT_PRESETS`、`PLATFORM_PRESET_MAP`、`StoreShotPreset` 类型、`listStoreShotPresets`、MCP `list_store_shot_presets`、`resolveBrandImageRef` 的 `name==="primary"` 回退；`save_brand_asset` 的 `{preset}` target 分支与 source-refine。零 shim。完成后全仓无残留引用（typecheck + grep 清零）。精确删除点见 SPEC-DATA-008。

### SPEC-BEHAVIOR-008 web 设置 + 画布 surface 分组
ProductDetail 后台页提供品牌资产设置（store_shot_count 下拉 3–8 / banner 勾选 / 3 个 poster 勾选）；保存 → `updateBrandAssetSettings`。BrandAssets 画布在 `groupByKind` 基础上按 `asset.surface` 再分子组，mobile/tablet 显示"Android…/iOS…"标签、其余端无 surface 标签；`banner` kind 自动成组。i18n `en`/`zh` 两块补齐键。

### SPEC-BEHAVIOR-009 agent 模板 + craft + CLAUDE.md 同步
三平台 `fm-app-icon`/`fm-brand-assets`/`fm-design` 同步更新且 `template-parity.test.ts` 通过；删三平台 `fm-rollback-design`（DesignView 回滚保留）；模板明确 fm-* 纯命令重跑＝全量重生成对应资产 + 执行前二次确认；`craft/image-prompts.md` 补 banner/poster-style/icon-母图 scaffold；根 `CLAUDE.md` 同步（banners kind、get_brand_asset_plan/delete_brand_asset、移除 list_store_shot_presets 与 name="primary" 描述、画布 surface 分组）。

## API / Data / Config Contracts

### SPEC-DATA-001 类型扩展（brand-assets.ts）
```ts
export type BrandSurface = "android" | "ios";
export const BRAND_ASSET_KINDS = ["app-icon", "store-shot", "banner", "poster"] as const; // +banner
const KIND_SUBDIR = { "app-icon": "app-icon", "store-shot": "store-shots", banner: "banners", poster: "posters" };
export interface BrandAssetRecord {
  kind: BrandAssetKind; name: string; files: BrandAssetFile[];
  brand_style: string; model?: string; generated_at: string;
  surface?: BrandSurface;   // NEW — omitted on web/desktop single-surface & poster
  variant?: string;         // NEW — icon layer name | poster style | undefined
}
```
`brandAssetRecordSchema`（zod `.strict()`）增 `surface: z.enum(["android","ios"]).optional()`、`variant: z.string().min(1).optional()`，`kind: z.enum(BRAND_ASSET_KINDS)` 自动含 banner。

### SPEC-DATA-002 product brand_assets 设置块（product.ts，DECISION-001：海报默认全开）
```ts
const brandAssetsSettingsSchema = z.object({
  store_shot_count: z.number().int().min(3).max(8).default(3),
  banner: z.boolean().default(false),
  poster_portrait: z.boolean().default(true),   // DECISION-001 A
  poster_landscape: z.boolean().default(true),  // DECISION-001 A
  poster_square: z.boolean().default(true),     // DECISION-001 A
}).strict();
// 注意：productSchema 形如 base.extend({...}).strict().superRefine(...)（结果是 ZodEffects，无 .extend()）。
// brand_assets 字段必须加进现有 .extend({...}) 的对象里（在 .strict() 之前），不可追加到链尾；
// brand_assets 为 optional，不参与 superRefine（语言/默认语言的既有联动）。
// productSchema 内: brand_assets: brandAssetsSettingsSchema.optional()
// productConfigSchema (单独 schema，后台写入用): 同字段 optional（同样加进其对象定义内）
```
`ProductService.updateBrandAssetSettings(productId: string, patch: Partial<BrandAssetsSettings>): Promise<Product>` 经 `runProductMutation({operation:"update-brand-asset-settings", product_id})`。读 product 时 `brand_assets` 缺省套 schema 默认。

### SPEC-DATA-003 计划类型（新模块，如 brand-asset-plan.ts）
```ts
export interface BrandAssetPlanEntry {
  kind: BrandAssetKind; surface?: BrandSurface; variant?: string;
  width: number; height: number; count: number;
  source?: string; verifiedAt?: string;        // provenance for verified sizes
  baseImages?: ("a" | "b" | "c")[];            // app-icon only
  variants?: string[];                          // app-icon only — derived variant names
}
export interface BrandAssetPlan { productId: string; platform: Platform; surfaces: BrandSurface[]; entries: BrandAssetPlanEntry[]; }
export function getBrandAssetPlan(product: Product): BrandAssetPlan;
```

### SPEC-DATA-004 分辨率表（结构固定，平台像素值 UNCONFIRMED 待核实）
每资产类型一张表，键 `(platform[,surface])`，值 `{width,height,source,verifiedAt}`：
- store-shot / banner / app-icon 的**平台尺寸为 UNCONFIRMED 候选**（源文档 §6.4 起点），实现期逐项核实官方文档后写入并附 source+verifiedAt；未核实值**不进测试断言**（断言仅覆盖键集/方向/count）。
- **poster 为受控常量，可断言**：portrait 1080×1920、landscape 1920×1080、square 1080×1080（平台无关，产品自有标准）。
- 桌面图标档（DECISION-001 A）：{1024,512,256,128,64,32,16}，丢弃源文档可疑的 358×358 / 720×72。
- Android 前景安全区（DECISION-001 A）：666×666（源文档 66×66 系笔误）。
沿用 `SHARP_PIXEL_LIMIT`(64MP) 与渲染 ≤16384 边界。

### SPEC-DATA-005 图标派生接口（brand-icon-derive.ts）
```ts
export interface DeriveIconInput {
  surface?: BrandSurface;                 // undefined ⇒ web/desktop single
  platform: Platform;
  logo: Buffer;       // 母图 a (透明 logo)
  background: Buffer; // 母图 b (不透明背景)
  safeLogo?: Buffer;  // 母图 c (666² 安全区 logo) — mobile/tablet 必需
  colors?: { mono?: string; tint?: string; dark_bg?: string };
}
export interface DerivedIconVariant { variant: string; width: number; height: number; png: Buffer; }
export function deriveAppIconVariants(input: DeriveIconInput): Promise<DerivedIconVariant[]>;
```
变体矩阵见 SPEC-BEHAVIOR-004。圆角＝SVG `<rect rx>` mask + `sharp().composite([{input: mask, blend:"dest-in"}])`。

### SPEC-DATA-006 save_brand_asset 输入（判别联合，brand-assets.ts + mcp/tools.ts schema）
```ts
type SaveBrandAssetInput =
  | { product_id; kind:"app-icon"; brand_style; platform: Platform;
      logo_ref:string; bg_ref:string; safe_logo_ref?:string; colors?:{...}; model? }
  | { product_id; kind:"store-shot"|"banner"|"poster"; name; brand_style;
      source:{html:string}; surface?:BrandSurface; variant?:string; target:{width;height}; model? };
// 返回类型亦为判别联合（按输入 kind）：
type SaveBrandAssetResult =
  | { kind:"app-icon"; assets: BrandAssetRecord[] }           // 整套派生记录
  | { kind:"store-shot"|"banner"|"poster"; asset: SavedBrandAsset }; // 单条
```
MCP `save_brand_asset` 的 zod source-refine（现 `image_ref XOR html` + `app-icon→image_ref`）整体替换为上述判别联合（输入与返回均按 kind 判别）。

### SPEC-DATA-007 MCP 工具集变更（mcp/tools.ts）
- 新增 `get_brand_asset_plan(product_id)` → `BrandAssetPlan`。
- 新增 `delete_brand_asset(product_id, kind, name)` → `{deleted:boolean}`。
- 移除 `list_store_shot_presets`。
- `save_brand_asset` schema 改 SPEC-DATA-006。
- 新工具**不读 media-config**；加无凭证字段外泄断言。

### SPEC-DATA-008 精确删除点（审计补全，PLAN 删除清单的权威来源）
- `packages/core/src/brand-assets.ts`：`STORE_SHOT_PRESETS`、`PLATFORM_PRESET_MAP`、`StoreShotPreset` 接口、`listStoreShotPresets`、`{preset}` target 分支（saveBrandAsset 内）、`resolveBrandImageRef` 的 `name==="primary"` 回退（:741）。
- `packages/core/src/store.ts`：`listStoreShotPresets` 转发方法（接口方法 :110 + 实现）、`StoreShotPreset` 的 value-import(:11) 与 type-import(:18) ——逐处清理，非笼统"~4 行"。
- `packages/mcp/src/tools.ts`：`list_store_shot_presets` 出现在 FORMA_TOOL_NAMES(:69)、schema map(:482)、description map(:522)、registration(:668)；以及 `save_brand_asset` source-refine(:345-380)。
- 测试：删除 `store-shot-presets.test.ts`（整文件）；`tools.test.ts` 的 `:6668-6790` describe 块 + `:277` mock + 相关断言；**并修 `packages/core/tests/brand-assets.test.ts`：移除 `STORE_SHOT_PRESETS` import(:31) 与用例(:630)**（否则 typecheck 断）。
- 文档：根 `CLAUDE.md` 中 `list_store_shot_presets` / `name="primary"` 描述。
> 注：审计确认仓内其余 "primary" 命中为 color-token / component-variant 等无关代码，**不得改动**。

**额外（resolveBrandImageRef bare-ref 选择规则，替换 MASTER_SIZE 依赖）**：移除 `name==="primary"` 回退后，bare ref `forma-image://brand/app-icon`（无 `@size`）的选择规则不再能按 `MASTER_SIZE=2048` 取（:747，2048 母版随 1080 工作画布管线消失）。改为确定规则：**选该 product app-icon 记录中 `variant` 为 standard 类、尺寸最大的那个文件**（web→standard 最大档、mobile/tablet→对应 surface 的 standard 最大档）；无匹配则 `FormaError`。需配套 TM 断言（见 TM-07 扩展）。

## External Documentation Checked

| Dependency | Version | Check Date | Conclusion |
|---|---|---|---|
| sharp (icon derivation) | 0.34.5 | 2026-06-14 | 已核实（审计 subagent，对照已装包）：greyscale / tint / composite blend:"dest-in" / alpha 保留 / resize / SVG mask 光栅化均支持 → SPEC-BEHAVIOR-004 可实现，无版本缺口 |
| OpenAI image model ids & sizes (gpt-image-2 / gpt-image-1.5) | UNCONFIRMED | 2026-06-14 | 本阶段未对官方文档核实；按 ASSUME-004/RISK-DEP-002 推迟到实现期，核实后写入 catalogue 并附 source+verifiedAt；未核实值不进断言 |
| Gemini image model ids & renderer 兼容性 (gemini-3-pro-image / gemini-3.1-flash-image) | UNCONFIRMED | 2026-06-14 | OpenAI 兼容 /images/generations 是否支持 Gemini 3 未核实；默认切换受 DECISION-002 A 门控（核实兼容后才切，否则保 2.5-flash 默认 + 原生 renderer 列后续） |
| 平台商店图/横幅/图标推荐分辨率 (Apple / Google Play) | UNCONFIRMED | 2026-06-14 | 沿用 STORE_SHOT_PRESETS 既有"官方来源 URL + verifiedAt"约定逐项核实；结构先行、数值后填；未核实值不进断言 |
| poster 三样式 / 桌面图标档 / Android 安全区 | N/A (受控常量) | 2026-06-14 | DECISION-001 A 已定（poster 1080×1920/1920×1080/1080×1080；桌面 {1024..16}；Android 安全区 666²）；产品自有标准，可直接断言 |

## Test Matrix
- **TM-01 catalogue**：4 新模型入 `IMAGE_MODELS`/`MODEL_SIZE_TABLES`；openai 默认=gpt-image-1.5；gemini 默认按门控（兼容→3.1-flash / 否→2.5-flash）；`resolveSize` 不抛；未核实像素值不被断言。
- **TM-02 surface 映射**：`brandSurfacesForPlatform` 四端返回值。
- **TM-03 plan 结构**：四端 × {store-shot,banner(on/off),poster(各开关),app-icon} 的 entries（surface 套数 / count=store_shot_count / variant 清单 / baseImages 集 / 方向），**不断言未核实像素**；poster 像素可断言。
- **TM-04 设置 schema**：store_shot_count 边界(2/3/8/9)、banner 默认 false、3 个 poster 默认 true；`updateBrandAssetSettings` 经锁；缺省套默认。
- **TM-05 记录/kind**：surface/variant 可选；banner ∈ kinds；KIND_SUBDIR/zod 同步；fixtures 更新。
- **TM-06 图标派生（不依赖 stub）**：最小 spike 起步——母图→各变体的尺寸/alpha/圆角/tint/greyscale 断言；缺 safeLogo 的 mobile/tablet 报错；web/desktop 仅 standard。
- **TM-07 save(app-icon) + bare-ref 解析**：判别联合输入派生整套并原子替换、返回 {assets}；重复保存替换不累积；`resolveBrandImageRef("forma-image://brand/app-icon")`（无 @size）按"standard 最大档"确定选择（不依赖 MASTER_SIZE），无匹配报 `FormaError`。
- **TM-08 delete_brand_asset**：删记录+文件；越界/绝对/`..` 路径拒绝；不存在报错；孤儿清理。
- **TM-09 破坏式移除**：typecheck + grep 全仓无 listStoreShotPresets/STORE_SHOT_PRESETS/PLATFORM_PRESET_MAP/StoreShotPreset/list_store_shot_presets/name==="primary"（无关 primary 除外）残留；旧测试已删。
- **TM-10 MCP**：get_brand_asset_plan / delete_brand_asset 注册与返回；save_brand_asset 新 schema；无凭证外泄断言。
- **TM-11 web**：ProductDetail 设置交互+保存；BrandAssets surface 子分组（mobile/tablet vs web/desktop）+ banner 组；i18n en/zh 键齐全。
- **TM-12 agent/craft/docs**：template-parity 通过；fm-rollback-design 三平台删除；fm-* 重跑语义在模板中存在；CLAUDE.md 同步。
- **TM-13 兜底**：`pnpm lint` / `pnpm typecheck` / `pnpm test` / `pnpm check:vzi-boundary` 全绿。

## Non-goals
- 不做数据迁移 / 兼容 shim（ASSUME-001）。
- 不把真实 provider 计费调用纳入 CI（仅 stub；真实模型仅人工验收）。
- 不在本批实现 Gemini 原生 generateContent renderer（DECISION-002 A，门控后续）。
- 不重做海报/横幅渲染模板美术（SCOPE-OUT-005）。
- 不动 DesignView 版本回滚（仅删 fm-rollback-design 命令模板）。
- 不新增 provider / 新运行时 / 新生图依赖。

## PLAN Handoff
PLAN 应据此切任务（建议顺序）：
1. 类型与 schema 骨架（SPEC-DATA-001/002/003）+ surface 映射（SPEC-BEHAVIOR-002）+ 单测 TM-02/04/05。
2. 分辨率表骨架（SPEC-DATA-004，结构先行、平台像素 UNCONFIRMED 占位；poster/桌面档/安全区按 DECISION-001 定值）。
3. getBrandAssetPlan（SPEC-BEHAVIOR-003）+ TM-03。
4. 图标派生 spike → 全变体（SPEC-BEHAVIOR-004 / SPEC-DATA-005）+ TM-06（不依赖 stub）。
5. save_brand_asset 判别联合 + delete_brand_asset（SPEC-BEHAVIOR-005/006 / SPEC-DATA-006）+ TM-07/08。
6. **破坏式移除**（SPEC-BEHAVIOR-007 / SPEC-DATA-008 权威删除清单）+ TM-09，配合 codegraph_impact 复核。
7. catalogue 扩充 + 模型/兼容性官方核实（SPEC-BEHAVIOR-001）+ TM-01；Gemini 默认按门控。
8. MCP 工具集（SPEC-DATA-007）+ TM-10。
9. web 设置 + 画布 surface 分组 + i18n（SPEC-BEHAVIOR-008）+ TM-11。
10. agent 模板 + craft + CLAUDE.md + fm-rollback-design 删除（SPEC-BEHAVIOR-009）+ TM-12。
11. 平台分辨率官方核实定稿（OVERFLOW-002，独立步骤，产出带 source URL 的表）。
12. 全量验证 TM-13。
人工验收（真实 provider）单列，不在自动路径。

## Trace
<!-- Map this stage's IDs to upstream/downstream. R3 derives & checks closure. -->
| This ID | Upstream | Status |
|---|---|---|
| SPEC-BEHAVIOR-001 | DES-ARCH-001, AC-001, DECISION-002 | active |
| SPEC-BEHAVIOR-002 | DES-ARCH-002, AC-002, RISK-CORR-001 | active |
| SPEC-BEHAVIOR-003 | DES-ARCH-005, AC-002 | active |
| SPEC-BEHAVIOR-004 | DES-ARCH-007, AC-004, RISK-DEP-003 | active |
| SPEC-BEHAVIOR-005 | DES-ARCH-008, AC-004 | active |
| SPEC-BEHAVIOR-006 | DES-ARCH-008, AC-005, RISK-SEC-002 | active |
| SPEC-BEHAVIOR-007 | DES-ARCH-005/009, AC-003, RISK-DATA-001 | active |
| SPEC-BEHAVIOR-008 | DES-ARCH-010/011, AC-008/009 | active |
| SPEC-BEHAVIOR-009 | DES-ARCH-012, AC-010/011, OQ-001/002 | active |
| SPEC-DATA-001 | DES-ARCH-003, AC-007 | active |
| SPEC-DATA-002 | DES-ARCH-004, AC-006, DECISION-001 | active |
| SPEC-DATA-003 | DES-ARCH-005 | active |
| SPEC-DATA-004 | DES-ARCH-006, RISK-DEP-002, DECISION-001 | active |
| SPEC-DATA-005 | DES-ARCH-007 | active |
| SPEC-DATA-006 | DES-ARCH-008 | active |
| SPEC-DATA-007 | DES-ARCH-009, RISK-SEC-001 | active |
| SPEC-DATA-008 | DES-ARCH-005/009, 审计补全 | active |
| TM-01..TM-13 | AC-001..AC-012 | active |
