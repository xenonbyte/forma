...
### SPEC-BEHAVIOR-001 移动端提示词直角禁令（←DES-PROMPT-001）
...
### SPEC-BEHAVIOR-002 platform 解析与平台视口渲染（←DES-LINT-001 步骤 0/1）
- `saveDesignArtifact`（`packages/core/src/design-save.ts`）在 Step 3 渲染前解析：`platform = input.forma.platform ?? (await products.getProduct(productId)).platform`；解析值（可能为 `undefined`，产品未配置 platform 时）：
  ...
  - `platform === "mobile"` 时 `renderArtifactPreview({ …, viewport: { width: 390, height: 884 } })`（对齐 `VIEWPORT_PRESETS.mobile`，`requirement-vzi-capture.ts` 同源）；其余平台与 undefined 维持默认 1280×800。
...
### SPEC-BEHAVIOR-003 rootCorners 快照契约（←DES-LINT-001 步骤 2）
...
### SPEC-BEHAVIOR-004 screen-edge-radius 检查语义（←DES-LINT-001 步骤 3/4）
- `LintOptions` 新增 `platform?: string`。`lintCraft` 返回数组**始终**包含 id 为 `screen-edge-radius` 的检查项（可观测性契约）：
...
### SPEC-BEHAVIOR-005 DesignView 纯画布页（←DES-WEB-001）
...
### SPEC-BEHAVIOR-006 路由归并（←DES-ROUTE-001）
...
### SPEC-BEHAVIOR-007 RequirementDetail 精简（←DES-WEB-002）
...
- 文档卡片右上角 actions（经 SPEC-BEHAVIOR-008 槽位）两个图标按钮（inline SVG + `aria-label` + `title`）：
  ...
  - **打开设计稿**：`<a href="/products/:pid/requirements/:rid/design">`；`ui_affected === false` **或** 该需求无 html artifact（`listProductArtifacts(productId, "html")` 过滤 `requirement_id`，排除 `design-system`/`component-library` 后为空）→ 禁用态（非链接渲染 + tooltip 文案）；否则正常导航。AC-005 无死链契约。
- 数据获取减为 `getRequirement` + `listProductArtifacts`（后者仅为禁用态判定）。

### SPEC-BEHAVIOR-008 WorkSurface actions 槽（←DES-WEB-002）
...
### SPEC-BEHAVIOR-009 管理端移动端 mock 直角化（←DES-WEB-003）
...
### SPEC-BEHAVIOR-010 i18n 键变更（←DES-WEB-002/001）
...
- 无新增第三方依赖、无新外部 API 面。`@xyflow/react` v12 用法（`onlyRenderVisibleElements`、`Background`、`setCenter`）完全沿用仓库内已验证代码（`Canvas.tsx:68` 注释注明 setCenter 已按 v12 核实）——不引入超出现有用法的 React Flow API。
...
| TEST-CORE-003 | 防空转护栏：mobile 产品保存设计 → craftChecks 含**非 skipped** 的 screen-edge-radius；mobile 渲染 viewport 为 390×884 | 集成（mock 渲染器或注入 deps） | 同上 |
| TEST-WEB-001 | DesignView：渲染画布容器；tile iframe `sandbox` 不含 `allow-scripts`；无 PNG `<img>` 网格 | 组件测试 | `web/src/pages/DesignView.test.tsx`（重写） |
...
| TEST-WEB-007 | PlatformTemplatePreview mobile mock：外层无 rounded 类、无 "9:41" | 组件测试 | 既有测试文件更新 |
...
|---|---|---|
| SPEC-BEHAVIOR-001 | DES-PROMPT-001 / AC-001 / SCOPE-IN-001 | specified |
| SPEC-BEHAVIOR-002 | DES-LINT-001(0/1) / 评审 v2 major#1#2 | specified |
| SPEC-BEHAVIOR-003 | DES-LINT-001(2) | specified |
| SPEC-BEHAVIOR-004 | DES-LINT-001(3/4) / DEC-004 / AC-001 | specified |
| SPEC-BEHAVIOR-005 | DES-WEB-001 / AC-002 / AC-005 / DEC-001 / MIT-001 / MIT-002 | specified |
| SPEC-BEHAVIOR-006 | DES-ROUTE-001 / DEC-002 / MIT-003 | specified |
| SPEC-BEHAVIOR-007 | DES-WEB-002 / AC-003 / AC-004 / AC-005 / DEC-003 | specified |
| SPEC-BEHAVIOR-008 | DES-WEB-002 | specified |
| SPEC-BEHAVIOR-009 | DES-WEB-003 / SCOPE-IN-002 | specified |
| SPEC-BEHAVIOR-010 | DES-WEB-002 / MIT-004 / 评审 v2 minor#3 | specified |
| TEST-PROMPT-001..002 | SPEC-BEHAVIOR-001 / AC-001 | specified |
| TEST-CORE-001..003 | SPEC-BEHAVIOR-002..004 | specified |
| TEST-WEB-001..007 | SPEC-BEHAVIOR-005..009 / AC-002..005 | specified |
| TEST-ALL | AC-006 | specified |
