...
- `packages/core/src/design-save.ts:147-168` — 保存管线渲染预览并跑 `lintCraft(renderResult.snapshot)`；`forma.platform` 在 `SaveDesignInput.forma` 可得（`design-save.ts:50`）。
...
`packages/od-contracts/src/prompts/system.ts`：cross-platform deliverable / product-realism 相关条目如引用 mobile 屏幕呈现，追加同一句式（实现时以 grep "device shell" 定位全部落点，保持措辞一致）。
...
1. `packages/core/src/quality/rendered-dom.ts`：
   ...
   - `extractSnapshotInPage()` 内（保持自包含）：取 `document.body` 与 body 的直接子元素中"覆盖视口"者（`getBoundingClientRect` 与视口交叠 ≥98% 宽且顶部 ≤2px 视为全幅根容器），读取 `getComputedStyle().borderTopLeftRadius` 等四值解析为 px，写入 `rootCorners`。旧快照无此字段 → 检查跳过（向后兼容，手工构建快照的测试不受影响）。
2. `packages/core/src/quality/craft-lint.ts`：
...
3. `packages/core/src/design-save.ts:157`：`lintCraft(renderResult.snapshot, { platform: forma.platform })`。
...
- i18n：新增 `action.copyDocument`、`action.copied`、`requirement.openDesignDisabled` 等键；移除不再引用的 `requirement.pages*`、`requirement.navigation*`、`design.artifactPreview`、`action.openInApp` 等死键（实现时 grep 确认无其他引用后删除）。
...
### DES-TEST-001 测试与验证
- od-contracts：`system-prompt.test.ts` 加固定语句断言。
- core：`craft-lint` 单测——手工快照含/不含 `rootCorners`、mobile/非 mobile、圆角/直角四象限；`rendered-dom` 提取逻辑由现有 preview 渲染测试路径覆盖（若有 puppeteer 测试则加用例，否则单测手建快照）。
- web：重写 `DesignView.test.tsx`（渲染 Canvas 容器、空态、`sandbox` 属性断言不含 `allow-scripts`〔MIT-001〕）；重写 `RequirementDetail.test.tsx`(仅文档+两按钮、复制成功反馈 mock clipboard、禁用态、不再渲染 pages/navigation/preview)；更新 `routes.test.ts`（viewer 路由移除）；删除 `ViewerPage.test.tsx`。
- 全量：`pnpm test` + `pnpm typecheck`。

## Decision Requests

none
（OQ-001→DEC-001 纯画布、OQ-002→DEC-002 重写 /design 删 /viewer、OQ-003→DEC-004 提示词+静态检查、OQ-004→DEC-003 移除深链，均已由用户在检查点确认。）
