...
### RISK-A11Y-001 BC1 白底改造对比度不达 WCAG AA
Status: open
...
- **MIT-008（→RISK-A11Y-001）**：BC1/BC2 改造同步适配点网（深点 on 白）/磨砂框/标签色，验收纳入 WCAG AA 对比检查；不新增 contrast-aa 失败（参照 `viewer-craft-contrast-debt`）。
...
| RISK-A11Y-001 | SCOPE-IN-014, SCOPE-IN-015 | covered |
