...
- **指针写入缺口（关键）**：`grep` 确认 `setDesignSystemArtifactPointerLocked` **零调用者**——`designSystemArtifactId` 从不被写入。`getBaselinePage`（`tools.ts:1225`）/`getBaselineImage`（:1244）/`get_product_baseline`（:420）读它 → 现网恒 `ARTIFACT_NOT_FOUND`。
...
| DECISION-001 | RISK-MIG-002, SCOPE-IN-007, SCOPE-IN-012 | pending |
