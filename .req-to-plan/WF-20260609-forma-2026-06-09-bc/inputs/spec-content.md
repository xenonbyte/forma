...
### SPEC-BEHAVIOR-001 最终命令集为 6 且安装一致（R1/R5 → DES-ARCH-001）
...
### SPEC-BEHAVIOR-002 MCP 工具集净变化 +1/−3（R4/R5/B3/B7 → DES-ARCH-001/006/007）
...
### SPEC-BEHAVIOR-003 产品删除仅经后台、core/HTTP/web 保留（R4 边界 → DES-ARCH-001）
...
### SPEC-BEHAVIOR-004 三平台去壳正文逐字一致（R2 → DES-ARCH-002）
...
### SPEC-BEHAVIOR-005 样板下沉后命令语义不变（R3 → DES-ARCH-002）
...
### SPEC-BEHAVIOR-006 命令门槛三档复用 core 错误码（B1 → DES-ARCH-003）
...
### SPEC-BEHAVIOR-007 fm-refine-components 产出产品设计系统（B2 → DES-ARCH-004）
...
### SPEC-BEHAVIOR-008 当前组件库指针：激活既有 designSystemArtifactId（B2/B7 核心 → DES-ARCH-005）
...
### SPEC-BEHAVIOR-009 数据驱动 baseline 规格与 get_component_baseline（B7 → DES-ARCH-006）
...
### SPEC-BEHAVIOR-010 get_design_context 扩展两字段（B7 → DES-ARCH-006）
...
### SPEC-BEHAVIOR-011 fm-change-style 产品级委托（B3 → DES-ARCH-007）
...
### SPEC-BEHAVIOR-012 fm-design 缺组件库两段式停下（B4 → DES-ARCH-008）
...
### SPEC-BEHAVIOR-013 fm-design 按需复用 + rule 1 不回溯（B5/B6 → DES-ARCH-008）
...
### SPEC-BEHAVIOR-014 web 版本/回退 UI 下线、current_version 内部保留（W1–W5 → DES-ARCH-009）
...
### SPEC-BEHAVIOR-015 web 无限画布白底+增强+品牌资源（BC1/BC2/BC3 → DES-ARCH-010）
...
### SPEC-DATA-001 manifest `forma.productIcon` 字段（DES-ARCH-004）
...
- `validateFormaExtension` 增分支：present 时校验 primary/monochrome 为合法 supporting path、shape 三字段为非空串；**缺字段容错**：读取面遇无 `productIcon` 返回"无 ICON"而非抛错（RISK-MIG-003）。

### SPEC-DATA-002 当前组件库指针写/读契约（DES-ARCH-005）
...
### SPEC-DATA-003 `component-baseline.ts` 数据形状（DES-ARCH-006）
```
...
`componentLibrary?: { artifactId: string; version: number; bundleUrl?: string; previewUrl?: string; productIcon?: { primary: string; monochrome: string; shape: {...} } }`。**结构化引用，不内联当前 HTML**；模型需完整 markup 时经既有 `get_product_artifact`/`export_artifact` 读取（满足 B7"或等价读取面"，且界定体量、解 RISK-CONTRACT-002）。无 `designSystemArtifactId` → 字段 `undefined`。

### SPEC-DATA-005 `get_component_baseline` MCP schema（DES-ARCH-006）
...
### SPEC-DATA-006 MCP / web 删除面接线清单（DES-ARCH-001/007/009）
...
- **React Flow / @xyflow**（BC2/BC3 画布增强所属外部库）：本 spec 的画布契约保持**引擎无关的行为级**表述（"逐 tile 标题标签存在""选中框可见""滚轮缩放+拖拽平移与标注一致"），不对 React Flow 版本特定 API（自定义 node、`selected` 态样式、viewport pan/zoom 控件）作断言。**实现期（PLAN/编码）须经 Context7 核实** xyflow 当前版本的 custom-node overlay 与 selection/viewport API；本阶段不引入未经核实的版本敏感声明（无 `UNCONFIRMED` 断言）。
- **CanvasKit / @vzi-core**（BC1）：仅改样式参数（底色/点网/磨砂框/标签色），不触渲染 API，无需外部文档核实。
...
|---|---|---|
| SPEC-BEHAVIOR-001 | DES-ARCH-001 | covered |
| SPEC-BEHAVIOR-002 | DES-ARCH-001, DES-ARCH-006, DES-ARCH-007 | covered |
| SPEC-BEHAVIOR-003 | DES-ARCH-001 | covered |
| SPEC-BEHAVIOR-004 | DES-ARCH-002 | covered |
| SPEC-BEHAVIOR-005 | DES-ARCH-002 | covered |
| SPEC-BEHAVIOR-006 | DES-ARCH-003 | covered |
| SPEC-BEHAVIOR-007 | DES-ARCH-004 | covered |
| SPEC-BEHAVIOR-008 | DES-ARCH-005 | covered |
| SPEC-BEHAVIOR-009 | DES-ARCH-006 | covered |
| SPEC-BEHAVIOR-010 | DES-ARCH-006 | covered |
| SPEC-BEHAVIOR-011 | DES-ARCH-007 | covered |
| SPEC-BEHAVIOR-012 | DES-ARCH-008 | covered |
| SPEC-BEHAVIOR-013 | DES-ARCH-008 | covered |
| SPEC-BEHAVIOR-014 | DES-ARCH-009 | covered |
| SPEC-BEHAVIOR-015 | DES-ARCH-010 | covered |
| SPEC-DATA-001 | DES-ARCH-004 | covered |
| SPEC-DATA-002 | DES-ARCH-005 | covered |
| SPEC-DATA-003 | DES-ARCH-006 | covered |
| SPEC-DATA-004 | DES-ARCH-006 | covered |
| SPEC-DATA-005 | DES-ARCH-006 | covered |
| SPEC-DATA-006 | DES-ARCH-001, DES-ARCH-007, DES-ARCH-009 | covered |
