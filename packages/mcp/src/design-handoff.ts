...
async function resolveAssetRef(
  ...
  if (!isSameOrChildPath(root, abs)) {
    ...
    throw new FormaError("ARTIFACT_INVALID_INPUT", "Resolved icon asset path escapes the artifact root", {
      artifactId,
    });
  }
  ...
  try {
    ...
    const err = cause as NodeJS.ErrnoException;
    if (err.code === "ENOENT") {
      throw new FormaError("ARTIFACT_NOT_FOUND", "Generated icon asset not found", {
        artifactId,
        handoffType: "icons",
        relativePath: rel,
      });
    }
    throw new FormaError("ARTIFACT_WRITE_FAIL", "Generated icon asset is unreadable", {
      artifactId,
      handoffType: "icons",
      relativePath: rel,
      cause: err.message,
...
async function attachAssetRefs(
  ...
  for (const el of elements) {
    ...
    if (!raw) continue;
    const assetRef = await resolveAssetRef(raw as { metadata?: Record<string, unknown> }, artifactDir, artifactId);
 ...
 */
export async function toolGetDesignHandoff(store: FormaStore, input: McpGetDesignHandoffInput) {
  ...
  const pages = await resolvePagePointers(store, productId, requirement_id, productsRoot, requirementPageIdSet(req));
 ...
 */
export async function toolGetPageUi(store: FormaStore, input: McpGetPageUiInput) {
  ...
  if (node_id) {
    ...
    if (!rootNode) {
      throw new FormaError("ARTIFACT_NOT_FOUND", `Node ${node_id} not found in page UI`, {
 ...
 */
export async function toolGetUiNode(store: FormaStore, input: McpGetUiNodeInput) {
  ...
  if (!element) {
    throw new FormaError("ARTIFACT_NOT_FOUND", `Node ${node_id} not found in VZI for page ${page_id}`, {
      requirement_id,
      page_id,
      node_id,
    });
 ...
 * search_page_ui — search elements by text/type.
 */
export async function toolSearchPageUi(store: FormaStore, input: McpSearchPageUiInput) {
  ...
  const artifactDir = getArtifactDir(productsRoot, productId, pageInfo.artifactId);
  const elementsWithAssetRef: Array<(typeof result.elements)[number] & { assetRef?: string }> = result.elements.map(
    (el) => ({ ...el }),
  );
  await attachAssetRefs(elementsWithAssetRef, content, artifactDir, pageInfo.artifactId);
