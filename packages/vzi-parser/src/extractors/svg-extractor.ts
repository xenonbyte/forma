...
export function extractSVGData(svgElement: SVGSVGElement): SVGData | undefined {
  ...
  function traverseElement(element: Element, inheritedFill?: string, inheritedStroke?: string): void {
    ...
    if (tagName === 'path') {
      ...
      if (d) {
        const fillRule = (element.getAttribute('fill-rule') || computedStyle.fillRule || 'nonzero') as 'nonzero' | 'evenodd';
        paths.push({
          d,
  ...
  if (paths.length === 0 && (!circles || circles.length === 0) &&
      (!rects || rects.length === 0) && (!polygons || polygons.length === 0)) {
    return undefined;
  }
