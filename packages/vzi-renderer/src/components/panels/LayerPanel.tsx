...
function buildLayerTree(
  ...
  const item: LayerItem = {
    ...
    children: children.flatMap((child) =>
      buildLayerTree(elements, child.id, depth + 1, expandedIds, visited)
    ),
...
export const LayerPanel: React.FC<LayerPanelProps> = memo(({
  ...
  // 拖拽开始
  const handleDragStart = useCallback((elementId: string) => (e: React.DragEvent) => {
  ...
  // 拖拽悬停
  const handleDragOver = useCallback((elementId: string) => (e: React.DragEvent) => {
    e.preventDefault();
  ...
  const handleDrop = useCallback((targetId: string) => (e: React.DragEvent) => {
    ...
    if (dropPosition === 'inside') {
      ...
      // 计算目标元素的子元素数量作为新索引
      const targetChildren = Object.values(elements).filter(
        (el) => el.parentId === targetId
      );
      ...
      <div>
        {flatList.map((item) => (
          <LayerItemRenderer
            ...
            dropPosition={
              dragState.dropTargetId === item.element.id
                ? dragState.dropPosition
                : null
            }
            onSelect={() => onElementSelect?.(item.element.id)}
            onHover={(isHovered) =>
              onElementHover?.(isHovered ? item.element.id : null)
            }
