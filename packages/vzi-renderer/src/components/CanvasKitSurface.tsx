...
function normalizeBounds(bounds: FlatElementBounds | undefined): { x: number; y: number; width: number; height: number } {
  return {
...
export function buildCanvasKitElementTree(ir: FlatIRDocumentLike | IntermediateRepresentation): IRElement[] {
  ...
  for (const [fallbackId, rawElement] of Object.entries(sourceElements)) {
    const id = (typeof rawElement.id === 'string' && rawElement.id.trim().length > 0)
      ? rawElement.id
      : fallbackId;
    const svgData = rawElement.svgData == null
      ? undefined
    ...
    const element: IRElement = {
      ...
      children: [],
      svgData,
      src: typeof rawElement.src === 'string'
        ? rawElement.src
...
export const CanvasKitSurface: React.FC<CanvasKitSurfaceProps> = memo(({
  ...
  const commitViewport = useCallback((next: CanvasKitViewportState) => {
  ...
  const updateViewport = useCallback((updater: (prev: CanvasKitViewportState) => CanvasKitViewportState) => {
    commitViewport(updater(viewportStateRef.current));
  }, [commitViewport]);

  const commitSelectedElement = useCallback((element: IRElement | null) => {
  ...
  const commitHoveredElement = useCallback((element: IRElement | null) => {
    const nextId = element?.id ?? null;
  ...
  const resolveCanvasPointOrCenter = useCallback((clientX: number, clientY: number): { x: number; y: number } | null => {
    const canvas = canvasRef.current;
    if (!canvas) {
      return null;
    }
  ...
  const handleRendererFailure = useCallback((error: Error) => {
  ...
  const findElementAtCanvasPoint = useCallback((canvasX: number, canvasY: number): IRElement | null => {
    ...
    for (let i = hitTestElements.length - 1; i >= 0; i -= 1) {
      ...
      if (
        worldX >= x &&
        worldX <= x + elementWidth &&
        worldY >= y &&
        worldY <= y + elementHeight
      ) {
        return el;
      }
    }
  ...
  useEffect(() => {
    ...
    requestRender();
  }, [rendererReady, requestRender, viewportState, activeHoveredId, activeSelectedId, renderElements, backgroundColor, showAnnotations]);
