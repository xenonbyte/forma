...
import { exportRequirementIcons, type ExportRequirementIconsDeps } from "../src/requirement-icon-export.js";
import { getArtifactVersionDir, getArtifactIconsDir, getArtifactIconsManifestPath } from "../src/artifact-paths.js";
...
function makePointer(requirementId: string, pageId: string, artifactId: string, version = 3): DesignPointer {
  return {
    requirementId,
    pageId,
    variant: "default",
...
describe("Case 1: multi-page filtering by requirementId", () => {
  it("only exports pointers matching the given requirementId", async () => {
    ...
    try {
      ...
      await scaffoldArtifactVersion(productsRoot, PRODUCT_ID, ARTIFACT_ID_1, 3, makeHtml([makeSvg(24, 24, "Logo")]));
      await scaffoldArtifactVersion(productsRoot, PRODUCT_ID, ARTIFACT_ID_2, 1, makeHtml([makeSvg(16, 16, "Arrow")]));
  ...
  it("skips active pointers whose page no longer exists on the requirement", async () => {
    ...
    try {
      ...
      await scaffoldArtifactVersion(productsRoot, PRODUCT_ID, ARTIFACT_ID_1, 3, makeHtml([makeSvg(24, 24, "Current")]));
  ...
  it("returns empty result when no pointers match requirementId", async () => {
    ...
    try {
      const deps = await makeTestDeps(home);
      deps.listDesignPointers = async () => [makePointer(OTHER_REQ_ID, PAGE_ID_1, ARTIFACT_ID_1)];
...
describe("Case 2: zero-icon page still writes icons.json", () => {
  it("creates icons/ with icons.json containing icons:[] when no SVGs", async () => {
    ...
    try {
      ...
      await scaffoldArtifactVersion(productsRoot, PRODUCT_ID, ARTIFACT_ID_1, 2, makeHtmlNoIcons());

      deps.listDesignPointers = async () => [makePointer(REQ_ID, PAGE_ID_1, ARTIFACT_ID_1, 2)];
...
describe("Case 3: stale icons/ is fully replaced", () => {
  it("removes stale files from a prior export", async () => {
    ...
    try {
      ...
      await scaffoldArtifactVersion(productsRoot, PRODUCT_ID, ARTIFACT_ID_1, 5, makeHtml([makeSvg(24, 24, "Home")]));
      ...
      deps.listDesignPointers = async () => [makePointer(REQ_ID, PAGE_ID_1, ARTIFACT_ID_1, 5)];
...
describe("Case 4: temp dir is cleaned up on extractor failure", () => {
  it("leaves no .tmp-* sibling when extractIconAssets throws", async () => {
    ...
    try {
      ...
      await scaffoldArtifactVersion(productsRoot, PRODUCT_ID, ARTIFACT_ID_1, 1, unsafeHtml);

      deps.listDesignPointers = async () => [makePointer(REQ_ID, PAGE_ID_1, ARTIFACT_ID_1, 1)];
      ...
      const artifactDir = join(productsRoot, PRODUCT_ID, "od-project", "artifacts", ARTIFACT_ID_1);
      let entries: string[] = [];
      try {
...
describe("Case 5: whole-export failure — one page error aborts everything", () => {
  it("does not commit page-1 icons when page-2 fails to read", async () => {
    ...
    try {
      ...
      // Page 1 is valid
      await scaffoldArtifactVersion(productsRoot, PRODUCT_ID, ARTIFACT_ID_1, 1, makeHtml([makeSvg(24, 24, "Logo")]));
...
describe("Case 6: result shape", () => {
  it("returns correct totalIcons across multiple pages", async () => {
    ...
    try {
      ...
      // Page 2: 1 icon
      await scaffoldArtifactVersion(productsRoot, PRODUCT_ID, ARTIFACT_ID_2, 2, makeHtml([makeSvg(32, 32, "C")]));
...
describe("Case 7: icons.json manifest content", () => {
  it("writes valid IconManifest JSON to icons/icons.json", async () => {
    ...
    try {
      ...
      await scaffoldArtifactVersion(productsRoot, PRODUCT_ID, ARTIFACT_ID_1, 4, makeHtml([makeSvg(24, 24, "Close")]));

      deps.listDesignPointers = async () => [makePointer(REQ_ID, PAGE_ID_1, ARTIFACT_ID_1, 4)];
  ...
  it("SVG files and PNG files are written under icons/", async () => {
    ...
    try {
      ...
      await scaffoldArtifactVersion(productsRoot, PRODUCT_ID, ARTIFACT_ID_1, 1, makeHtml([makeSvg(24, 24, "Star")]));
