...
async function seedLegacyArtifact(
  ...
  const manifest = {
    ...
    id: artifactId,
    kind,
    renderer: kind === "design-system" ? "design-system" : "html",
    title: "Legacy",
    entry: "index.html",
    status: "complete",
    exports: ["index.html"],
...
describe("A6 backfill", () => {
  ...
  it("recovers an interrupted migration where v1 exists but flat manifest cleanup did not finish", async () => {
    ...
    await writeFile(
      join(artifactDir, "v1", "manifest.json"),
      JSON.stringify({
        version: 1,
        id: "AbCdEfGhIjKlMnOp",
        kind: "design-page",
    ...
    await backfillDesignArtifacts({
      ...
      productMutationLock: {
        run: async (input, fn) => {
          ...
          return fn({ operation: input.operation, product_id: input.product_id, warnings: [] });
        },
      },
    });
