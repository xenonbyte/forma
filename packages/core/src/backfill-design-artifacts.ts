...
async function backfillDesignArtifactsLocked(
  home: string,
  ...
  try {
    productIds = (await readdir(productsRoot, { withFileTypes: true }))
      .filter((e) => e.isDirectory() && /^P-[a-f0-9]{6}$/.test(e.name))
      .map((e) => e.name);
  } catch {
    return;
  }
  ...
  for (const productId of productIds) {
    ...
    for (const artifactId of artifactIds) {
      ...
      if (!validation.ok) {
        report.notes.push(`skip ${artifactId}: ${validation.error}`);
        report.skipped += 1;
        continue;
      }
...
async function recoverExistingV1(
  ...
  if (!validation.ok) {
    report.notes.push(`blocked ${artifactId}: existing v1 invalid: ${validation.error}`);
    report.skipped += 1;
    return "invalid";
  }
...
async function moveLegacyDirIntoV1(
  artifactDir: string,
  versionDir: string,
  migratedManifest: ArtifactManifest,
): Promise<void> {
...
async function maybeSetPointer(
  products: ProductService,
  productId: string,
  artifactId: string,
  ...
  if (
    manifest.kind !== "design-page" ||
    !manifest.forma?.requirementId ||
    !manifest.forma.pageId ||
    !manifest.forma.variant
  )
    return false;
  const current = await products.getDesignPointer(
    productId,
    manifest.forma.requirementId,
    manifest.forma.pageId,
    manifest.forma.variant,
