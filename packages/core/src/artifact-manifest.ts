...
export const ALLOWED_RENDERERS = ["html", "design-system", "markdown", "svg", "image", "preview-only"] as const;
...
export function validateFormaExtension(forma: unknown): FormaValidationResult {
  ...
  if (f["preview"] !== undefined) {
    ...
    if (
      typeof p !== "object" ||
      p === null ||
      !(PREVIEW_STATUSES as readonly string[]).includes(p["status"] as string)
    ) {
  ...
  if (
    f["provenance"] !== undefined &&
    (typeof f["provenance"] !== "object" || f["provenance"] === null || Array.isArray(f["provenance"]))
  ) {
  ...
  if (f["quality"] !== undefined) {
    ...
    if (typeof q !== "object" || q === null || Array.isArray(q))
      return { ok: false, error: "forma.quality must be an object" };
...
type ValidationResult = { ok: true; value: ArtifactManifest } | { ok: false; error: string };
