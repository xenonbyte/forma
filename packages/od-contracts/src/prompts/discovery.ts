...
- **iOS app**: create a dedicated iOS product file/screen (for example \`mobile-ios.html\`) with the screen content only (no iPhone frame, no notch or pill chrome, no status bar, no gesture bar, square outer corners — no rounded screen edges), 44px minimum hit targets, iOS-safe bottom navigation or sheet patterns, and no Android-only Material navigation.
- **Android app**: create a dedicated Android product file/screen (for example \`mobile-android.html\`) with the screen content only (no Pixel frame, status bar, or nav bar chrome, square outer corners — no rounded screen edges), 48dp hit targets, Material navigation patterns, and no iOS-only chrome.
