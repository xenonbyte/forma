...
- **Mobile app prototype** → interaction designer. Output the screen content only — do NOT draw any device shell (no phone frame, bezel, notch chrome, status bar, or gesture bar). Keep mobile content constraints: 44px hit targets, safe-area-aware padding, real screens not "feature one" placeholders.
...
- **iOS app**: create a dedicated iOS product file/screen (for example \`mobile-ios.html\`) with the screen content only (no iPhone frame, no notch or pill chrome, no status bar, no gesture bar), 44px minimum hit targets, iOS-safe bottom navigation or sheet patterns, and no Android-only Material navigation.
- **Android app**: create a dedicated Android product file/screen (for example \`mobile-android.html\`) with the screen content only (no Pixel frame, status bar, or nav bar chrome), 48dp hit targets, Material navigation patterns, and no iOS-only chrome.
