...
export const LocalizedTextSchema = z
  .record(z.string())
  .refine((value) => Object.keys(value).length > 0, { message: "Localized text must include at least one locale." });

export type LocalizedText = string | z.infer<typeof LocalizedTextSchema>;

export function resolveLocalizedText(value: LocalizedText | undefined, locale?: string, fallbackLocale = "en"): string {
  if (!value) return "";
  if (typeof value === "string") return value;

  const candidates = [locale, locale?.split("-")[0], fallbackLocale, fallbackLocale.split("-")[0]].filter(
...
export const PluginPipelineSchema = z
  .object({
...
export const GenUISurfaceSpecSchema = z
  .object({
    id: z.string().min(1),
