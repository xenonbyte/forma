  ...
  describe("daemon mode (no streamFormat)", () => {
    ...
    it("does not instruct agents to ask for a second visual-direction picker", () => {
      ...
      expect(prompt).toContain(
        "if a design system is active and no new brand/reference source was provided, use it as the visual direction without asking again",
      );
    });
    ...
    it("uses stable brand option values for discovery-form branching", () => {
      ...
      expect(prompt).toContain(
        "active-design-system cases where the user did not provide a new brand/reference source",
      );
