...
import { SpatialIndexBuilder, VZIDecoder, VZIEncoder, type VZIContent } from "@vzi-core/format";
import {
  rewriteResourceUrl,
...
describe("rewriteResourceUrl", () => {
  ...
  it("keeps data: URLs", () => {
    const errs: unknown[] = [];
    expect(rewriteResourceUrl("data:image/png;base64,xx", undefined, URLS, ctx, errs as never)).toBe(
  ...
  it("uses metadata.iconRelativePath when present", () => {
    const errs: never[] = [];
    expect(rewriteResourceUrl("data:x", { iconRelativePath: "icons/star.svg" }, URLS, ctx, errs)).toBe(
      `${URLS.iconBaseUrl}star.svg`,
    );
  });
  it("rewrites assets/ via bundleBaseUrl", () => {
    const errs: never[] = [];
    expect(rewriteResourceUrl("assets/pic.png", undefined, URLS, ctx, errs)).toBe(
      `${URLS.bundleBaseUrl}assets/pic.png`,
    );
  });
...
describe("composeAnnotationCanvas", () => {
      ...
      return makeContent(
        [
          { id: "root", parentId: null, type: "container", bounds: { x: 0, y: 0, width: 100, height: 50 }, styles: {} },
          {
            ...
            parentId: "root",
            type: "text",
            bounds: { x: 8, y: 12, width: 60, height: 20 },
            styles: {},
            textContent: label,
          },
        ],
    ...
    const result = composeAnnotationCanvas(
      [
        ...
        },
        {
          ...
          variant: "default",
          title: "Settings",
          content: duplicateContent("Settings"),
          urls: URLS,
        },
      ],
      10,
  ...
  it("rewrites content.images refs and uses metadata asset IDs", () => {
    ...
    const result = composeAnnotationCanvas([
      { pageId: "home", artifactId: "A", variant: "default", title: "Home", content, urls: URLS },
    ]);
  ...
  it("rewrites and tracks CSS background-image URLs via the bundle base URL", () => {
    ...
    expect(result.errors).toHaveLength(0);
    expect(result.elements[0].styles.backgroundImage).toBe(
      `linear-gradient(#000, #111), url("${URLS.bundleBaseUrl}assets/hero.png")`,
    );
    expect(result.resourceRefs).toEqual([
      {
        artifactId: "A",
  ...
  it("rewrites VZI file: image and background URLs captured from artifact HTML", () => {
    const content = makeContent(
      [
        ...
        },
        {
          ...
          styles: {
            backgroundImage:
              'url("file:///Users/xubo/.forma/products/P-abc123/od-project/artifacts/A/v1/assets/hero.png")',
          },
        },
      ],
    ...
    expect(result.elements.find((e) => e.id === "hero")?.styles.backgroundImage).toBe(
      `url("${URLS.bundleBaseUrl}assets/hero.png")`,
    );
  ...
  it("decodes real VZI bytes and builds a non-empty CanvasKit element tree", () => {
    const source = makeContent(
      [
        { id: "root", parentId: null, type: "container", bounds: { x: 0, y: 0, width: 320, height: 640 }, styles: {} },
        {
          ...
          parentId: "root",
          type: "text",
          bounds: { x: 16, y: 24, width: 200, height: 32 },
          styles: {},
          textContent: "Home",
        },
      ],
    ...
    const result = composeAnnotationCanvas([
      { pageId: "home", artifactId: "A", variant: "default", title: "Home", content: decodeResult.content, urls: URLS },
    ]);
    expect(result.elements.length).toBeGreaterThan(0);
  });
