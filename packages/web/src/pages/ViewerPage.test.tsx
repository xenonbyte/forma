...
describe("ViewerPage", () => {
  it("loads requirement + artifacts and renders Viewer with a built model (requirement entry)", async () => {
    ...
    const model = viewerSpy.mock.calls[0][0].model as {
      __model: { entry: string; artifacts: Array<{ artifactId: string }> };
    };
  ...
  it("filters to a single page for the page entry", async () => {
    ...
    await act(async () => {
      root.render(
        <ViewerPage client={fakeClient()} params={{ productId: "p1", reqId: "r1", pageId: "login" }} entry="page" />,
      );
      await flushPromises();
    });

    expect(container.querySelector("[data-testid='viewer']")).toBeTruthy();
    const model = viewerSpy.mock.calls.at(-1)![0].model as {
      __model: { entry: string; artifacts: Array<{ artifactId: string }> };
    };
