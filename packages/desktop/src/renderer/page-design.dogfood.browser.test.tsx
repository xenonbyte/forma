// P9.6 dogfood — 页面设计画布 (page design canvas).
// Drives the real AppShell into a PAGE selection (hash deep-link) so the page nav
// item is active in the shell chrome, then lints the chrome. Distinct from the
// requirement canvas: the active nav item is the page, and the topbar crumb is
// "需求 / 页面" (requirement / page).
//
// The <Viewer> is MOCKED. design/annotation mode is internal to <Viewer>
// (top toggle [data-action="mode-design"] / [data-action="mode-annotation"]).
// FALLBACK (verified): the REAL <Viewer> fails contrast-aa on its own chrome —
// "React Flow" 2.85:1 (@xyflow/react attribution, packages/viewer/src/Canvas.tsx)
// and "标注功能待后续版本提供。" 3.54:1 (AnnotationSlot #888,
// packages/viewer/src/AnnotationSlot.tsx). Both are @xenonbyte/forma-viewer
// concerns, not the P9.5 shell tokens; out of scope, so the viewer stays mocked.
...
describe("dogfood: 页面设计画布", () => {
  it("shell chrome (sidebar/topbar + page crumb) passes all craft-lint rules", async () => {
    installForma();
    window.location.hash = "#/products/p1/requirements/r1/pages/login";
    const container = render(<AppShell />);
    await flush();
    await flush();

    // Page chrome is mounted: the page nav item is active.
    const active = container.querySelector('[data-nav-page="login"].sidebar__item--active');
