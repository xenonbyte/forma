import { OPEN_DESIGN_PLUGIN_SPEC_VERSION, type PluginManifest } from "@xenonbyte/od-contracts";
...
export function adaptClaudePlugin(rawJson: string, opts: ClaudePluginAdapterOptions): ClaudePluginAdapterResult {
  ...
  if (commands > 0) {
    warnings.push(
      `claude-plugin declares ${commands} command(s); v1 OD apply does not auto-register hooks. Add them via od.context.claudePlugins[].`,
    );
  }
