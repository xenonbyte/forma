...
2. Inspect `manifest.forma.quality.craftChecks`. For any check with `passed:false` (e.g. `contrast-aa`, `type-scale`, `color-palette`, `font-families`, `no-em-dash`, `eyebrow-density`): fix that specific violation in the HTML, call the appropriate save tool again — Forma points the artifact to the corrected design and supersedes the old version. Repeat until all checks pass.
3. **Visual check (mandatory when the platform can view images):** open the rendered preview and look at it — on disk at `$FORMA_HOME/data/products/<product_id>/od-project/artifacts/<artifact_id>/preview/2x.png` (`FORMA_HOME` defaults to `~/.forma`), or via the returned `preview_url` when the Forma server is running. Confirm the render matches intent: no clipped or overlapping text, no broken layout, imagery renders, hierarchy reads as designed. If the preview is unavailable (`preview_status` pending/failed) or the platform cannot view images, state that in the report instead of skipping silently.
4. Also judge the non-mechanical craft items the lint cannot catch: the result matches the stated design read and dials (`craft/design-read.md`); no production AI tells (`craft/ai-tells.md` — em/en-dashes in Latin copy, fake-precise numbers, decorative status dots, theme/accent/radius inconsistency); functional icons all come from the icon library (`search_icons` / Lucide), none hand-drawn; generated illustrations were each Read-inspected and are on-palette (consistent with the brand tokens); copy self-audit done on every visible string; avoid AI-slop layouts; clear type hierarchy; restrained color (accent reserved for primary actions); WCAG AA contrast on real backgrounds; empty/loading/error states present; forms show inline validation; purposeful motion; honor core UX laws. Regenerate if any fails.
