...
4. Confirm the `brand_style` and optional `system_style` (read product config / `list_styles`); confirm with the user when not already chosen.
5. Fetch the style knowledge BEFORE generating: `get_style(brand_style)` returns DESIGN.md (design principles), tokens.css (design tokens), components.html (reference components). If a `system_style` is set, `get_style(system_style)` too. Component libraries have no requirement, so requirement page context does not apply here.
6. Call `get_component_baseline(product_id)` to obtain the authoritative spec: `{ platform, baseline: { foundations, components } }`. Use this as the sole source of WHAT to produce — the foundations category list and the platform's component list with states/variants. Do NOT transcribe or hardcode the component list from any other source; use the tool's return value.
