...
7. Generate or refine the component library as **shared CSS (`tokens_css`) plus an ordered list of units (`units`)**. Do NOT produce a single combined HTML document; instead produce:
   ...
   **Functional icons (hard rule):** never hand-draw functional icons. Call `search_icons` and inline the returned Lucide SVG (currentColor inheritance; stroke-width follows tokens). Decorative brand-specific glyphs defined by tokens are the only exception. (This does not apply to the product ICON mark in unit b, which is created/recolored per the ICON rules above.)
