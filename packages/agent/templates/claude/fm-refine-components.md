...
6. Generate or refine the component library as one self-contained static HTML document. The document must contain three sections in order:
   ...
   b. **Product ICON** — primary (full-color) and monochrome variants, derived from the product name and brand_style. ICON rules:
      ...
      - For INITIAL generation, or for REFINEMENT when `manifest.forma.productIcon` or `manifest.forma.productIcon.shape` is missing: treat only the ICON as initial icon creation, create a new SVG mark, and assign a stable `shape_id` (a short slug, e.g. `<productName>-mark`). Preserve the existing `source_html` component structure when `source_html` exists.
