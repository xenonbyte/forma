...
2. Read current context before AI merge with MCP: `get_requirement` for current document, pages, translations, and design metadata; `get_product_baseline` for baseline pages and navigation; `get_product` for language config including `languages` and `default_language`; `get_product_rules` for product rules.
...
9. Detect direct contradiction, condition coverage conflict, and missing dependency against product rules after excluding rules whose `source_requirement` is the current requirement id.
10. Show page changes, generated rules, and conflicts to the user.
11. Convert confirmed conflict overrides into `replaces_rule_id` on the generated rule that replaces the old rule.
12. Convert confirmed deletions into `remove_rule_ids`; also include rules associated with confirmed `remove_page_ids`.
13. Call `save_requirement` with the validated document, `ui_affected`, pages, navigation, translations, rules, `remove_page_ids`, `remove_rule_ids`, and replacement metadata. Report stable error codes when returned.
