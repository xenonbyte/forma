...
7. Regenerate or initially generate the component library as one self-contained static HTML document, applying the new style tokens. The document must contain three sections in order:
   ...
   b. **Product ICON** — if a current manifest was loaded in step 4 and `manifest.forma.productIcon.shape` exists, recover the existing persisted `shape` (`shapeId` + `geometry` + `sourceVersion`). Reuse that geometry exactly and only recolor per the new brand tokens. Keep `shapeId` stable. When saving with `generate_components`, convert the persisted camelCase fields to the tool's snake_case input fields. Do NOT redraw the mark from scratch during re-skin. If no current library existed, or if `manifest.forma.productIcon` or `manifest.forma.productIcon.shape` is missing, treat only the ICON as initial icon creation: create a new SVG mark, assign a stable `shape_id`, render both variants, and preserve the existing `source_html` component structure when `source_html` exists. Render both primary and monochrome variants inline AND prepare them as separate SVG files for `supporting_files`.
   c. **Baseline component set** — every component in `baseline.components` for the product's platform with all listed states/variants. When `source_html` exists, modify it in place, preserving existing structure and content, applying only new tokens. When no `source_html` exists, generate the baseline component set from scratch.
