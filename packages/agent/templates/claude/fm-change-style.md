...
4. Load the current component library for re-skin when one exists (inlined fm-refine-components flow):
   ...
   - If a current library exists, call `get_product_artifact(product_id, artifact_id)` to read its manifest (recover `forma.productIcon.shape` for ICON geometry reuse).
...
7. Regenerate or initially generate the component library as one self-contained static HTML document, applying the new style tokens. The document must contain three sections in order:
   ...
   b. **Product ICON** — if a current manifest was loaded in step 4, read `manifest.forma.productIcon` to recover the existing persisted `shape` (`shapeId` + `geometry` + `sourceVersion`). Reuse that geometry exactly and only recolor per the new brand tokens. Keep `shapeId` stable. When saving with `generate_components`, convert the persisted camelCase fields to the tool's snake_case input fields. Do NOT redraw the mark from scratch during re-skin. If no current library existed, create a new SVG mark and assign a stable `shape_id`. Render both primary and monochrome variants inline AND prepare them as separate SVG files for `supporting_files`.
