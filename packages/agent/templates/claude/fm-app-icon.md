...
9. Report the derived size set (the `files[]` paths + dimensions) and the brand-assets canvas URL `#/products/<product_id>/brand-assets` where the icon renders. Report stable error codes exactly as returned. The real codes reachable in this flow: `MEDIA_NOT_CONFIGURED` (image model not configured, from `generate_image`); `BRAND_ASSET_INVALID_INPUT` (from `save_brand_asset`, e.g. `details.reason: "product_not_configured"` when platform/brand_style are missing).
