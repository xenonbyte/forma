...
- The product must be configured (`platform` + `brand_style`). If the config is missing, the core tool returns `PRODUCT_NOT_CONFIGURED` — STOP and guide the user to run `init_product_config` (or the backstage config) before retrying. The per-platform icon size set is derived from `platform`, so this is required.
...
9. Report the derived size set (the `files[]` paths + dimensions) and the brand-assets canvas URL `#/products/<product_id>/brand-assets` where the icon renders. Report stable error codes exactly as returned.
