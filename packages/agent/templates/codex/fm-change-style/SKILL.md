...
7. Regenerate the component library as one self-contained static HTML document, applying the new style tokens. The document must contain three sections in order:
   ...
   b. **Product ICON** — read `manifest.forma.productIcon` (from step 4) to recover the existing persisted `shape` (`shapeId` + `geometry` + `sourceVersion`). Reuse that geometry exactly and only recolor per the new brand tokens. Keep `shapeId` stable. When saving with `generate_components`, convert the persisted camelCase fields to the tool's snake_case input fields. Do NOT redraw the mark from scratch. Render both primary and monochrome variants inline AND prepare them as separate SVG files for `supporting_files`.
